...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> ydsps=Languages();RecordCount=ydsps.Count;int ydsp_index=1;
foreach (DB.LebiShop.Lebi_Language ydsp in ydsps){%>
            <dd <%if (ydsp_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=ydsp.id%>,'<%=ydsp.Code%>','<%=ydsp.Path%>');"><%if (ydsp.ImageUrl!=""){%><img src="<%=Image(ydsp.ImageUrl) %>" /><%}%><%=ydsp.Name%></a></dd>
            <%ydsp_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int qrRL_index=1;
List<Lebi_Currency> qrRLs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency qrRL in qrRLs){%>
            <dd <%if (qrRL_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=qrRL.id%>,'<%=qrRL.Code%>',<%=qrRL.ExchangeRate%>,'<%=qrRL.Msige%>','<%=qrRL.DecimalLength%>');"><%=qrRL.Code%></a></dd>
            <%qrRL_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int uaQY_index=1;
List<Lebi_Searchkey> uaQYs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey uaQY in uaQYs){%><%=Lang(uaQY.Name)%><%uaQY_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int GAcv_index=1;
List<Lebi_Searchkey> GAcvs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey GAcv in GAcvs){%>
<%if (GAcv.Type==1){ %><a href="<%=URL("P_Search",""+Lang(GAcv.Name)+"") %>"><%}else{ %><a href="<%=GAcv.URL%>" target="_blank"><%} %><span><%=Lang(GAcv.Name)%></span></a>
<%GAcv_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int XUyj_index=1;
List<Lebi_Page> XUyjs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page XUyj in XUyjs){%>
<a class="menu" href="<%=URL("","",XUyj.url)%>" target="<%=XUyj.target%>"><span style="color:<%=XUyj.NameColor%>"><%=XUyj.Name%></span></a> 
<%XUyj_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int OaCz_index=1;
List<Lebi_FriendLink> OaCzs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink OaCz in OaCzs){%>
            
                <% if (OaCz.Logo != "" && OaCz.IsPic == 1){ %><a href="<%=OaCz.Url%>" target="_blank"><img src="<%=Image(OaCz.Logo) %>" alt="<%=OaCz.Name%>" /></a><%}else{%><a href="<%=OaCz.Url%>" target="_blank"><%=OaCz.Name%></a><%} %>     
