...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> ufNRs=Languages();RecordCount=ufNRs.Count;int ufNR_index=1;
foreach (DB.LebiShop.Lebi_Language ufNR in ufNRs){%>
            <dd <%if (ufNR_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=ufNR.id%>,'<%=ufNR.Code%>','<%=ufNR.Path%>');"><%if (ufNR.ImageUrl!=""){%><img src="<%=Image(ufNR.ImageUrl) %>" /><%}%><%=ufNR.Name%></a></dd>
            <%ufNR_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int EahS_index=1;
List<Lebi_Currency> EahSs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency EahS in EahSs){%>
            <dd <%if (EahS_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=EahS.id%>,'<%=EahS.Code%>',<%=EahS.ExchangeRate%>,'<%=EahS.Msige%>','<%=EahS.DecimalLength%>');"><%=EahS.Code%></a></dd>
            <%EahS_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int dlua_index=1;
List<Lebi_Searchkey> dluas = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey dlua in dluas){%><%=Lang(dlua.Name)%><%dlua_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int uLUP_index=1;
List<Lebi_Searchkey> uLUPs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey uLUP in uLUPs){%>
<%if (uLUP.Type==1){ %><a href="<%=URL("P_Search",""+Lang(uLUP.Name)+"") %>"><%}else{ %><a href="<%=uLUP.URL%>" target="_blank"><%} %><span><%=Lang(uLUP.Name)%></span></a>
<%uLUP_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int KUkA_index=1;
List<Lebi_Page> KUkAs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page KUkA in KUkAs){%>
<a class="menu" href="<%=URL("","",KUkA.url)%>" target="<%=KUkA.target%>"><span style="color:<%=KUkA.NameColor%>"><%=KUkA.Name%></span></a> 
<%KUkA_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int dtnz_index=1;
List<Lebi_FriendLink> dtnzs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink dtnz in dtnzs){%>
            
                <% if (dtnz.Logo != "" && dtnz.IsPic == 1){ %><a href="<%=dtnz.Url%>" target="_blank"><img src="<%=Image(dtnz.Logo) %>" alt="<%=dtnz.Name%>" /></a><%}else{%><a href="<%=dtnz.Url%>" target="_blank"><%=dtnz.Name%></a><%} %>     
