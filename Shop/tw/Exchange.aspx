...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> OSqss=Languages();RecordCount=OSqss.Count;int OSqs_index=1;
foreach (DB.LebiShop.Lebi_Language OSqs in OSqss){%>
            <dd <%if (OSqs_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=OSqs.id%>,'<%=OSqs.Code%>','<%=OSqs.Path%>');"><%if (OSqs.ImageUrl!=""){%><img src="<%=Image(OSqs.ImageUrl) %>" /><%}%><%=OSqs.Name%></a></dd>
            <%OSqs_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int mnoR_index=1;
List<Lebi_Currency> mnoRs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency mnoR in mnoRs){%>
            <dd <%if (mnoR_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=mnoR.id%>,'<%=mnoR.Code%>',<%=mnoR.ExchangeRate%>,'<%=mnoR.Msige%>','<%=mnoR.DecimalLength%>');"><%=mnoR.Code%></a></dd>
            <%mnoR_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int CUYb_index=1;
List<Lebi_Searchkey> CUYbs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey CUYb in CUYbs){%><%=Lang(CUYb.Name)%><%CUYb_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int mcvw_index=1;
List<Lebi_Searchkey> mcvws = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey mcvw in mcvws){%>
<%if (mcvw.Type==1){ %><a href="<%=URL("P_Search",""+Lang(mcvw.Name)+"") %>"><%}else{ %><a href="<%=mcvw.URL%>" target="_blank"><%} %><span><%=Lang(mcvw.Name)%></span></a>
<%mcvw_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int hPNT_index=1;
List<Lebi_Page> hPNTs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page hPNT in hPNTs){%>
<a class="menu" href="<%=URL("","",hPNT.url)%>" target="<%=hPNT.target%>"><span style="color:<%=hPNT.NameColor%>"><%=hPNT.Name%></span></a> 
<%hPNT_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int iBbA_index=1;
List<Lebi_Page> iBbAs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page iBbA in iBbAs){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",iBbA.id,iBbA.url)%>" target="<%=iBbA.target%>" title="<%=iBbA.Name%>"><span style="color:<%=iBbA.NameColor%>"><%=iBbA.Name%></span></a></li>
        <%iBbA_index++;}%>
    </ul>
</div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int QlOa_index=1;
List<Lebi_FriendLink> QlOas = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink QlOa in QlOas){%>
            
                <% if (QlOa.Logo != "" && QlOa.IsPic == 1){ %><a href="<%=QlOa.Url%>" target="_blank"><img src="<%=Image(QlOa.Logo) %>" alt="<%=QlOa.Name%>" /></a><%}else{%><a href="<%=QlOa.Url%>" target="_blank"><%=QlOa.Name%></a><%} %>     
