...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> WHGXs=Languages();RecordCount=WHGXs.Count;int WHGX_index=1;
foreach (DB.LebiShop.Lebi_Language WHGX in WHGXs){%>
            <dd <%if (WHGX_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=WHGX.id%>,'<%=WHGX.Code%>','<%=WHGX.Path%>');"><%if (WHGX.ImageUrl!=""){%><img src="<%=Image(WHGX.ImageUrl) %>" /><%}%><%=WHGX.Name%></a></dd>
            <%WHGX_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int hRWS_index=1;
List<Lebi_Currency> hRWSs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency hRWS in hRWSs){%>
            <dd <%if (hRWS_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=hRWS.id%>,'<%=hRWS.Code%>',<%=hRWS.ExchangeRate%>,'<%=hRWS.Msige%>','<%=hRWS.DecimalLength%>');"><%=hRWS.Code%></a></dd>
            <%hRWS_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int RQXX_index=1;
List<Lebi_Searchkey> RQXXs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey RQXX in RQXXs){%><%=Lang(RQXX.Name)%><%RQXX_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int hzRO_index=1;
List<Lebi_Searchkey> hzROs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey hzRO in hzROs){%>
<%if (hzRO.Type==1){ %><a href="<%=URL("P_Search",""+Lang(hzRO.Name)+"") %>"><%}else{ %><a href="<%=hzRO.URL%>" target="_blank"><%} %><span><%=Lang(hzRO.Name)%></span></a>
<%hzRO_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int CeuO_index=1;
List<Lebi_Page> CeuOs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page CeuO in CeuOs){%>
<a class="menu" href="<%=URL("","",CeuO.url)%>" target="<%=CeuO.target%>"><span style="color:<%=CeuO.NameColor%>"><%=CeuO.Name%></span></a> 
<%CeuO_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int DJmW_index=1;
List<Lebi_Page> DJmWs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page DJmW in DJmWs){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",DJmW.id,DJmW.url)%>" target="<%=DJmW.target%>" title="<%=DJmW.Name%>"><span style="color:<%=DJmW.NameColor%>"><%=DJmW.Name%></span></a></li>
        <%DJmW_index++;}%>
    </ul>
</div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int lHcw_index=1;
List<Lebi_FriendLink> lHcws = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink lHcw in lHcws){%>
            
                <% if (lHcw.Logo != "" && lHcw.IsPic == 1){ %><a href="<%=lHcw.Url%>" target="_blank"><img src="<%=Image(lHcw.Logo) %>" alt="<%=lHcw.Name%>" /></a><%}else{%><a href="<%=lHcw.Url%>" target="_blank"><%=lHcw.Name%></a><%} %>     
