...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> qfVQs=Languages();RecordCount=qfVQs.Count;int qfVQ_index=1;
foreach (DB.LebiShop.Lebi_Language qfVQ in qfVQs){%>
            <dd <%if (qfVQ_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=qfVQ.id%>,'<%=qfVQ.Code%>','<%=qfVQ.Path%>');"><%if (qfVQ.ImageUrl!=""){%><img src="<%=Image(qfVQ.ImageUrl) %>" /><%}%><%=qfVQ.Name%></a></dd>
            <%qfVQ_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int ayCW_index=1;
List<Lebi_Currency> ayCWs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency ayCW in ayCWs){%>
            <dd <%if (ayCW_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=ayCW.id%>,'<%=ayCW.Code%>',<%=ayCW.ExchangeRate%>,'<%=ayCW.Msige%>','<%=ayCW.DecimalLength%>');"><%=ayCW.Code%></a></dd>
            <%ayCW_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int BvyM_index=1;
List<Lebi_Searchkey> BvyMs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey BvyM in BvyMs){%><%=Lang(BvyM.Name)%><%BvyM_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Hjof_index=1;
List<Lebi_Searchkey> Hjofs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Hjof in Hjofs){%>
<%if (Hjof.Type==1){ %><a href="<%=URL("P_Search",""+Lang(Hjof.Name)+"") %>"><%}else{ %><a href="<%=Hjof.URL%>" target="_blank"><%} %><span><%=Lang(Hjof.Name)%></span></a>
<%Hjof_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int DBpm_index=1;
List<Lebi_Page> DBpms = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page DBpm in DBpms){%>
<a class="menu" href="<%=URL("","",DBpm.url)%>" target="<%=DBpm.target%>"><span style="color:<%=DBpm.NameColor%>"><%=DBpm.Name%></span></a> 
<%DBpm_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int lLNy_index=1;
List<Lebi_FriendLink> lLNys = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink lLNy in lLNys){%>
            
                <% if (lLNy.Logo != "" && lLNy.IsPic == 1){ %><a href="<%=lLNy.Url%>" target="_blank"><img src="<%=Image(lLNy.Logo) %>" alt="<%=lLNy.Name%>" /></a><%}else{%><a href="<%=lLNy.Url%>" target="_blank"><%=lLNy.Name%></a><%} %>     
