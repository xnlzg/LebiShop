...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> KMLes=Languages();RecordCount=KMLes.Count;int KMLe_index=1;
foreach (DB.LebiShop.Lebi_Language KMLe in KMLes){%>
            <dd <%if (KMLe_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=KMLe.id%>,'<%=KMLe.Code%>','<%=KMLe.Path%>');"><%if (KMLe.ImageUrl!=""){%><img src="<%=Image(KMLe.ImageUrl) %>" /><%}%><%=KMLe.Name%></a></dd>
            <%KMLe_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int rQzD_index=1;
List<Lebi_Currency> rQzDs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency rQzD in rQzDs){%>
            <dd <%if (rQzD_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=rQzD.id%>,'<%=rQzD.Code%>',<%=rQzD.ExchangeRate%>,'<%=rQzD.Msige%>','<%=rQzD.DecimalLength%>');"><%=rQzD.Code%></a></dd>
            <%rQzD_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int iwor_index=1;
List<Lebi_Searchkey> iwors = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey iwor in iwors){%><%=Lang(iwor.Name)%><%iwor_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int vvPV_index=1;
List<Lebi_Searchkey> vvPVs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey vvPV in vvPVs){%>
<%if (vvPV.Type==1){ %><a href="<%=URL("P_Search",""+Lang(vvPV.Name)+"") %>"><%}else{ %><a href="<%=vvPV.URL%>" target="_blank"><%} %><span><%=Lang(vvPV.Name)%></span></a>
<%vvPV_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int RkVU_index=1;
List<Lebi_Page> RkVUs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page RkVU in RkVUs){%>
<a class="menu" href="<%=URL("","",RkVU.url)%>" target="<%=RkVU.target%>"><span style="color:<%=RkVU.NameColor%>"><%=RkVU.Name%></span></a> 
<%RkVU_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int zDBF_index=1;
List<Lebi_FriendLink> zDBFs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink zDBF in zDBFs){%>
            
                <% if (zDBF.Logo != "" && zDBF.IsPic == 1){ %><a href="<%=zDBF.Url%>" target="_blank"><img src="<%=Image(zDBF.Logo) %>" alt="<%=zDBF.Name%>" /></a><%}else{%><a href="<%=zDBF.Url%>" target="_blank"><%=zDBF.Name%></a><%} %>     
