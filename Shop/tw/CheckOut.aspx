...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> kWRgs=Languages();RecordCount=kWRgs.Count;int kWRg_index=1;
foreach (DB.LebiShop.Lebi_Language kWRg in kWRgs){%>
            <dd <%if (kWRg_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=kWRg.id%>,'<%=kWRg.Code%>','<%=kWRg.Path%>');"><%if (kWRg.ImageUrl!=""){%><img src="<%=Image(kWRg.ImageUrl) %>" /><%}%><%=kWRg.Name%></a></dd>
            <%kWRg_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int weuK_index=1;
List<Lebi_Currency> weuKs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency weuK in weuKs){%>
            <dd <%if (weuK_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=weuK.id%>,'<%=weuK.Code%>',<%=weuK.ExchangeRate%>,'<%=weuK.Msige%>','<%=weuK.DecimalLength%>');"><%=weuK.Code%></a></dd>
            <%weuK_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int hSJO_index=1;
List<Lebi_Searchkey> hSJOs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey hSJO in hSJOs){%><%=Lang(hSJO.Name)%><%hSJO_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int qeVB_index=1;
List<Lebi_Searchkey> qeVBs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey qeVB in qeVBs){%>
<%if (qeVB.Type==1){ %><a href="<%=URL("P_Search",""+Lang(qeVB.Name)+"") %>"><%}else{ %><a href="<%=qeVB.URL%>" target="_blank"><%} %><span><%=Lang(qeVB.Name)%></span></a>
<%qeVB_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int EbMc_index=1;
List<Lebi_Page> EbMcs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page EbMc in EbMcs){%>
<a class="menu" href="<%=URL("","",EbMc.url)%>" target="<%=EbMc.target%>"><span style="color:<%=EbMc.NameColor%>"><%=EbMc.Name%></span></a> 
<%EbMc_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int yRym_index=1;
List<Lebi_FriendLink> yRyms = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink yRym in yRyms){%>
            
                <% if (yRym.Logo != "" && yRym.IsPic == 1){ %><a href="<%=yRym.Url%>" target="_blank"><img src="<%=Image(yRym.Logo) %>" alt="<%=yRym.Name%>" /></a><%}else{%><a href="<%=yRym.Url%>" target="_blank"><%=yRym.Name%></a><%} %>     
