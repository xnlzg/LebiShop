...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> Bjrgs=Languages();RecordCount=Bjrgs.Count;int Bjrg_index=1;
foreach (DB.LebiShop.Lebi_Language Bjrg in Bjrgs){%>
            <dd <%if (Bjrg_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=Bjrg.id%>,'<%=Bjrg.Code%>','<%=Bjrg.Path%>');"><%if (Bjrg.ImageUrl!=""){%><img src="<%=Image(Bjrg.ImageUrl) %>" /><%}%><%=Bjrg.Name%></a></dd>
            <%Bjrg_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int gbUL_index=1;
List<Lebi_Currency> gbULs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency gbUL in gbULs){%>
            <dd <%if (gbUL_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=gbUL.id%>,'<%=gbUL.Code%>',<%=gbUL.ExchangeRate%>,'<%=gbUL.Msige%>','<%=gbUL.DecimalLength%>');"><%=gbUL.Code%></a></dd>
            <%gbUL_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int rgmU_index=1;
List<Lebi_Searchkey> rgmUs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey rgmU in rgmUs){%><%=Lang(rgmU.Name)%><%rgmU_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int MssB_index=1;
List<Lebi_Searchkey> MssBs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey MssB in MssBs){%>
<%if (MssB.Type==1){ %><a href="<%=URL("P_Search",""+Lang(MssB.Name)+"") %>"><%}else{ %><a href="<%=MssB.URL%>" target="_blank"><%} %><span><%=Lang(MssB.Name)%></span></a>
<%MssB_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int ztST_index=1;
List<Lebi_Page> ztSTs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page ztST in ztSTs){%>
<a class="menu" href="<%=URL("","",ztST.url)%>" target="<%=ztST.target%>"><span style="color:<%=ztST.NameColor%>"><%=ztST.Name%></span></a> 
<%ztST_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int iQlb_index=1;
List<Lebi_FriendLink> iQlbs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink iQlb in iQlbs){%>
            
                <% if (iQlb.Logo != "" && iQlb.IsPic == 1){ %><a href="<%=iQlb.Url%>" target="_blank"><img src="<%=Image(iQlb.Logo) %>" alt="<%=iQlb.Name%>" /></a><%}else{%><a href="<%=iQlb.Url%>" target="_blank"><%=iQlb.Name%></a><%} %>     
