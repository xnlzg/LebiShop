...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> kAcqs=Languages();RecordCount=kAcqs.Count;int kAcq_index=1;
foreach (DB.LebiShop.Lebi_Language kAcq in kAcqs){%>
            <dd <%if (kAcq_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=kAcq.id%>,'<%=kAcq.Code%>','<%=kAcq.Path%>');"><%if (kAcq.ImageUrl!=""){%><img src="<%=Image(kAcq.ImageUrl) %>" /><%}%><%=kAcq.Name%></a></dd>
            <%kAcq_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int aeKm_index=1;
List<Lebi_Currency> aeKms = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency aeKm in aeKms){%>
            <dd <%if (aeKm_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=aeKm.id%>,'<%=aeKm.Code%>',<%=aeKm.ExchangeRate%>,'<%=aeKm.Msige%>','<%=aeKm.DecimalLength%>');"><%=aeKm.Code%></a></dd>
            <%aeKm_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int fEMa_index=1;
List<Lebi_Searchkey> fEMas = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey fEMa in fEMas){%><%=Lang(fEMa.Name)%><%fEMa_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int QOin_index=1;
List<Lebi_Searchkey> QOins = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey QOin in QOins){%>
<%if (QOin.Type==1){ %><a href="<%=URL("P_Search",""+Lang(QOin.Name)+"") %>"><%}else{ %><a href="<%=QOin.URL%>" target="_blank"><%} %><span><%=Lang(QOin.Name)%></span></a>
<%QOin_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int tyNt_index=1;
List<Lebi_Page> tyNts = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page tyNt in tyNts){%>
<a class="menu" href="<%=URL("","",tyNt.url)%>" target="<%=tyNt.target%>"><span style="color:<%=tyNt.NameColor%>"><%=tyNt.Name%></span></a> 
<%tyNt_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int YKUP_index=1;
List<Lebi_FriendLink> YKUPs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink YKUP in YKUPs){%>
            
                <% if (YKUP.Logo != "" && YKUP.IsPic == 1){ %><a href="<%=YKUP.Url%>" target="_blank"><img src="<%=Image(YKUP.Logo) %>" alt="<%=YKUP.Name%>" /></a><%}else{%><a href="<%=YKUP.Url%>" target="_blank"><%=YKUP.Name%></a><%} %>     
