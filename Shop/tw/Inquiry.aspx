...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> VRSMs=Languages();RecordCount=VRSMs.Count;int VRSM_index=1;
foreach (DB.LebiShop.Lebi_Language VRSM in VRSMs){%>
            <dd <%if (VRSM_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=VRSM.id%>,'<%=VRSM.Code%>','<%=VRSM.Path%>');"><%if (VRSM.ImageUrl!=""){%><img src="<%=Image(VRSM.ImageUrl) %>" /><%}%><%=VRSM.Name%></a></dd>
            <%VRSM_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int EDKu_index=1;
List<Lebi_Currency> EDKus = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency EDKu in EDKus){%>
            <dd <%if (EDKu_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=EDKu.id%>,'<%=EDKu.Code%>',<%=EDKu.ExchangeRate%>,'<%=EDKu.Msige%>','<%=EDKu.DecimalLength%>');"><%=EDKu.Code%></a></dd>
            <%EDKu_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int bdkp_index=1;
List<Lebi_Searchkey> bdkps = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey bdkp in bdkps){%><%=Lang(bdkp.Name)%><%bdkp_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int hYus_index=1;
List<Lebi_Searchkey> hYuss = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey hYus in hYuss){%>
<%if (hYus.Type==1){ %><a href="<%=URL("P_Search",""+Lang(hYus.Name)+"") %>"><%}else{ %><a href="<%=hYus.URL%>" target="_blank"><%} %><span><%=Lang(hYus.Name)%></span></a>
<%hYus_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int TzVa_index=1;
List<Lebi_Page> TzVas = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page TzVa in TzVas){%>
<a class="menu" href="<%=URL("","",TzVa.url)%>" target="<%=TzVa.target%>"><span style="color:<%=TzVa.NameColor%>"><%=TzVa.Name%></span></a> 
<%TzVa_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int Mhvg_index=1;
List<Lebi_FriendLink> Mhvgs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink Mhvg in Mhvgs){%>
            
                <% if (Mhvg.Logo != "" && Mhvg.IsPic == 1){ %><a href="<%=Mhvg.Url%>" target="_blank"><img src="<%=Image(Mhvg.Logo) %>" alt="<%=Mhvg.Name%>" /></a><%}else{%><a href="<%=Mhvg.Url%>" target="_blank"><%=Mhvg.Name%></a><%} %>     
