...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> lJwAs=Languages();RecordCount=lJwAs.Count;int lJwA_index=1;
foreach (DB.LebiShop.Lebi_Language lJwA in lJwAs){%>
            <dd <%if (lJwA_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=lJwA.id%>,'<%=lJwA.Code%>','<%=lJwA.Path%>');"><%if (lJwA.ImageUrl!=""){%><img src="<%=Image(lJwA.ImageUrl) %>" /><%}%><%=lJwA.Name%></a></dd>
            <%lJwA_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int Daxx_index=1;
List<Lebi_Currency> Daxxs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency Daxx in Daxxs){%>
            <dd <%if (Daxx_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=Daxx.id%>,'<%=Daxx.Code%>',<%=Daxx.ExchangeRate%>,'<%=Daxx.Msige%>','<%=Daxx.DecimalLength%>');"><%=Daxx.Code%></a></dd>
            <%Daxx_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int nssT_index=1;
List<Lebi_Searchkey> nssTs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey nssT in nssTs){%><%=Lang(nssT.Name)%><%nssT_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int saIC_index=1;
List<Lebi_Searchkey> saICs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey saIC in saICs){%>
<%if (saIC.Type==1){ %><a href="<%=URL("P_Search",""+Lang(saIC.Name)+"") %>"><%}else{ %><a href="<%=saIC.URL%>" target="_blank"><%} %><span><%=Lang(saIC.Name)%></span></a>
<%saIC_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int SOzU_index=1;
List<Lebi_Page> SOzUs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page SOzU in SOzUs){%>
<a class="menu" href="<%=URL("","",SOzU.url)%>" target="<%=SOzU.target%>"><span style="color:<%=SOzU.NameColor%>"><%=SOzU.Name%></span></a> 
<%SOzU_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int OdzQ_index=1;
List<Lebi_FriendLink> OdzQs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink OdzQ in OdzQs){%>
            
                <% if (OdzQ.Logo != "" && OdzQ.IsPic == 1){ %><a href="<%=OdzQ.Url%>" target="_blank"><img src="<%=Image(OdzQ.Logo) %>" alt="<%=OdzQ.Name%>" /></a><%}else{%><a href="<%=OdzQ.Url%>" target="_blank"><%=OdzQ.Name%></a><%} %>     
