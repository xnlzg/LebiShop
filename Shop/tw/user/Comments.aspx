...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> Oxchs=Languages();RecordCount=Oxchs.Count;int Oxch_index=1;
foreach (DB.LebiShop.Lebi_Language Oxch in Oxchs){%>
            <dd <%if (Oxch_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=Oxch.id%>,'<%=Oxch.Code%>','<%=Oxch.Path%>');"><%if (Oxch.ImageUrl!=""){%><img src="<%=Image(Oxch.ImageUrl) %>" /><%}%><%=Oxch.Name%></a></dd>
            <%Oxch_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int jVhs_index=1;
List<Lebi_Currency> jVhss = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency jVhs in jVhss){%>
            <dd <%if (jVhs_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=jVhs.id%>,'<%=jVhs.Code%>',<%=jVhs.ExchangeRate%>,'<%=jVhs.Msige%>','<%=jVhs.DecimalLength%>');"><%=jVhs.Code%></a></dd>
            <%jVhs_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int xAFW_index=1;
List<Lebi_Searchkey> xAFWs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey xAFW in xAFWs){%><%=Lang(xAFW.Name)%><%xAFW_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int yIAp_index=1;
List<Lebi_Searchkey> yIAps = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey yIAp in yIAps){%>
<%if (yIAp.Type==1){ %><a href="<%=URL("P_Search",""+Lang(yIAp.Name)+"") %>"><%}else{ %><a href="<%=yIAp.URL%>" target="_blank"><%} %><span><%=Lang(yIAp.Name)%></span></a>
<%yIAp_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int WzSl_index=1;
List<Lebi_Page> WzSls = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page WzSl in WzSls){%>
<a class="menu" href="<%=URL("","",WzSl.url)%>" target="<%=WzSl.target%>"><span style="color:<%=WzSl.NameColor%>"><%=WzSl.Name%></span></a> 
<%WzSl_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int aUsy_index=1;
List<Lebi_FriendLink> aUsys = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink aUsy in aUsys){%>
            
                <% if (aUsy.Logo != "" && aUsy.IsPic == 1){ %><a href="<%=aUsy.Url%>" target="_blank"><img src="<%=Image(aUsy.Logo) %>" alt="<%=aUsy.Name%>" /></a><%}else{%><a href="<%=aUsy.Url%>" target="_blank"><%=aUsy.Name%></a><%} %>     
