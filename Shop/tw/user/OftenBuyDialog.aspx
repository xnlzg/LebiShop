...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> OXCrs=Languages();RecordCount=OXCrs.Count;int OXCr_index=1;
foreach (DB.LebiShop.Lebi_Language OXCr in OXCrs){%>
            <dd <%if (OXCr_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=OXCr.id%>,'<%=OXCr.Code%>','<%=OXCr.Path%>');"><%if (OXCr.ImageUrl!=""){%><img src="<%=Image(OXCr.ImageUrl) %>" /><%}%><%=OXCr.Name%></a></dd>
            <%OXCr_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int DbHK_index=1;
List<Lebi_Currency> DbHKs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency DbHK in DbHKs){%>
            <dd <%if (DbHK_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=DbHK.id%>,'<%=DbHK.Code%>',<%=DbHK.ExchangeRate%>,'<%=DbHK.Msige%>','<%=DbHK.DecimalLength%>');"><%=DbHK.Code%></a></dd>
            <%DbHK_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int aIXJ_index=1;
List<Lebi_Searchkey> aIXJs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey aIXJ in aIXJs){%><%=Lang(aIXJ.Name)%><%aIXJ_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int tJvf_index=1;
List<Lebi_Searchkey> tJvfs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey tJvf in tJvfs){%>
<%if (tJvf.Type==1){ %><a href="<%=URL("P_Search",""+Lang(tJvf.Name)+"") %>"><%}else{ %><a href="<%=tJvf.URL%>" target="_blank"><%} %><span><%=Lang(tJvf.Name)%></span></a>
<%tJvf_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int eFUP_index=1;
List<Lebi_Page> eFUPs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page eFUP in eFUPs){%>
<a class="menu" href="<%=URL("","",eFUP.url)%>" target="<%=eFUP.target%>"><span style="color:<%=eFUP.NameColor%>"><%=eFUP.Name%></span></a> 
<%eFUP_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int PGGT_index=1;
List<Lebi_FriendLink> PGGTs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink PGGT in PGGTs){%>
            
                <% if (PGGT.Logo != "" && PGGT.IsPic == 1){ %><a href="<%=PGGT.Url%>" target="_blank"><img src="<%=Image(PGGT.Logo) %>" alt="<%=PGGT.Name%>" /></a><%}else{%><a href="<%=PGGT.Url%>" target="_blank"><%=PGGT.Name%></a><%} %>     
