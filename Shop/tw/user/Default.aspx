...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> zRgls=Languages();RecordCount=zRgls.Count;int zRgl_index=1;
foreach (DB.LebiShop.Lebi_Language zRgl in zRgls){%>
            <dd <%if (zRgl_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=zRgl.id%>,'<%=zRgl.Code%>','<%=zRgl.Path%>');"><%if (zRgl.ImageUrl!=""){%><img src="<%=Image(zRgl.ImageUrl) %>" /><%}%><%=zRgl.Name%></a></dd>
            <%zRgl_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int Kybi_index=1;
List<Lebi_Currency> Kybis = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency Kybi in Kybis){%>
            <dd <%if (Kybi_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=Kybi.id%>,'<%=Kybi.Code%>',<%=Kybi.ExchangeRate%>,'<%=Kybi.Msige%>','<%=Kybi.DecimalLength%>');"><%=Kybi.Code%></a></dd>
            <%Kybi_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int zrrd_index=1;
List<Lebi_Searchkey> zrrds = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey zrrd in zrrds){%><%=Lang(zrrd.Name)%><%zrrd_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int imKQ_index=1;
List<Lebi_Searchkey> imKQs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey imKQ in imKQs){%>
<%if (imKQ.Type==1){ %><a href="<%=URL("P_Search",""+Lang(imKQ.Name)+"") %>"><%}else{ %><a href="<%=imKQ.URL%>" target="_blank"><%} %><span><%=Lang(imKQ.Name)%></span></a>
<%imKQ_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int vEpi_index=1;
List<Lebi_Page> vEpis = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page vEpi in vEpis){%>
<a class="menu" href="<%=URL("","",vEpi.url)%>" target="<%=vEpi.target%>"><span style="color:<%=vEpi.NameColor%>"><%=vEpi.Name%></span></a> 
<%vEpi_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int vAvJ_index=1;
List<Lebi_FriendLink> vAvJs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink vAvJ in vAvJs){%>
            
                <% if (vAvJ.Logo != "" && vAvJ.IsPic == 1){ %><a href="<%=vAvJ.Url%>" target="_blank"><img src="<%=Image(vAvJ.Logo) %>" alt="<%=vAvJ.Name%>" /></a><%}else{%><a href="<%=vAvJ.Url%>" target="_blank"><%=vAvJ.Name%></a><%} %>     
