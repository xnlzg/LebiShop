...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> oztFs=Languages();RecordCount=oztFs.Count;int oztF_index=1;
foreach (DB.LebiShop.Lebi_Language oztF in oztFs){%>
            <dd <%if (oztF_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=oztF.id%>,'<%=oztF.Code%>','<%=oztF.Path%>');"><%if (oztF.ImageUrl!=""){%><img src="<%=Image(oztF.ImageUrl) %>" /><%}%><%=oztF.Name%></a></dd>
            <%oztF_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int hWsw_index=1;
List<Lebi_Currency> hWsws = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency hWsw in hWsws){%>
            <dd <%if (hWsw_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=hWsw.id%>,'<%=hWsw.Code%>',<%=hWsw.ExchangeRate%>,'<%=hWsw.Msige%>','<%=hWsw.DecimalLength%>');"><%=hWsw.Code%></a></dd>
            <%hWsw_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int vEUx_index=1;
List<Lebi_Searchkey> vEUxs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey vEUx in vEUxs){%><%=Lang(vEUx.Name)%><%vEUx_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int youP_index=1;
List<Lebi_Searchkey> youPs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey youP in youPs){%>
<%if (youP.Type==1){ %><a href="<%=URL("P_Search",""+Lang(youP.Name)+"") %>"><%}else{ %><a href="<%=youP.URL%>" target="_blank"><%} %><span><%=Lang(youP.Name)%></span></a>
<%youP_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int apMf_index=1;
List<Lebi_Page> apMfs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page apMf in apMfs){%>
<a class="menu" href="<%=URL("","",apMf.url)%>" target="<%=apMf.target%>"><span style="color:<%=apMf.NameColor%>"><%=apMf.Name%></span></a> 
<%apMf_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int bujy_index=1;
List<Lebi_FriendLink> bujys = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink bujy in bujys){%>
            
                <% if (bujy.Logo != "" && bujy.IsPic == 1){ %><a href="<%=bujy.Url%>" target="_blank"><img src="<%=Image(bujy.Logo) %>" alt="<%=bujy.Name%>" /></a><%}else{%><a href="<%=bujy.Url%>" target="_blank"><%=bujy.Name%></a><%} %>     
