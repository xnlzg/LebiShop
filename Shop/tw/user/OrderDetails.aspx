...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> hLpCs=Languages();RecordCount=hLpCs.Count;int hLpC_index=1;
foreach (DB.LebiShop.Lebi_Language hLpC in hLpCs){%>
            <dd <%if (hLpC_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=hLpC.id%>,'<%=hLpC.Code%>','<%=hLpC.Path%>');"><%if (hLpC.ImageUrl!=""){%><img src="<%=Image(hLpC.ImageUrl) %>" /><%}%><%=hLpC.Name%></a></dd>
            <%hLpC_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int rurd_index=1;
List<Lebi_Currency> rurds = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency rurd in rurds){%>
            <dd <%if (rurd_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=rurd.id%>,'<%=rurd.Code%>',<%=rurd.ExchangeRate%>,'<%=rurd.Msige%>','<%=rurd.DecimalLength%>');"><%=rurd.Code%></a></dd>
            <%rurd_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int IFHS_index=1;
List<Lebi_Searchkey> IFHSs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey IFHS in IFHSs){%><%=Lang(IFHS.Name)%><%IFHS_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int UaRg_index=1;
List<Lebi_Searchkey> UaRgs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey UaRg in UaRgs){%>
<%if (UaRg.Type==1){ %><a href="<%=URL("P_Search",""+Lang(UaRg.Name)+"") %>"><%}else{ %><a href="<%=UaRg.URL%>" target="_blank"><%} %><span><%=Lang(UaRg.Name)%></span></a>
<%UaRg_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int FmkL_index=1;
List<Lebi_Page> FmkLs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page FmkL in FmkLs){%>
<a class="menu" href="<%=URL("","",FmkL.url)%>" target="<%=FmkL.target%>"><span style="color:<%=FmkL.NameColor%>"><%=FmkL.Name%></span></a> 
<%FmkL_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int EawL_index=1;
List<Lebi_FriendLink> EawLs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink EawL in EawLs){%>
            
                <% if (EawL.Logo != "" && EawL.IsPic == 1){ %><a href="<%=EawL.Url%>" target="_blank"><img src="<%=Image(EawL.Logo) %>" alt="<%=EawL.Name%>" /></a><%}else{%><a href="<%=EawL.Url%>" target="_blank"><%=EawL.Name%></a><%} %>     
