...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> JLPys=Languages();RecordCount=JLPys.Count;int JLPy_index=1;
foreach (DB.LebiShop.Lebi_Language JLPy in JLPys){%>
            <dd <%if (JLPy_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=JLPy.id%>,'<%=JLPy.Code%>','<%=JLPy.Path%>');"><%if (JLPy.ImageUrl!=""){%><img src="<%=Image(JLPy.ImageUrl) %>" /><%}%><%=JLPy.Name%></a></dd>
            <%JLPy_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int cSqG_index=1;
List<Lebi_Currency> cSqGs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency cSqG in cSqGs){%>
            <dd <%if (cSqG_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=cSqG.id%>,'<%=cSqG.Code%>',<%=cSqG.ExchangeRate%>,'<%=cSqG.Msige%>','<%=cSqG.DecimalLength%>');"><%=cSqG.Code%></a></dd>
            <%cSqG_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Exaa_index=1;
List<Lebi_Searchkey> Exaas = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Exaa in Exaas){%><%=Lang(Exaa.Name)%><%Exaa_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int GvFp_index=1;
List<Lebi_Searchkey> GvFps = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey GvFp in GvFps){%>
<%if (GvFp.Type==1){ %><a href="<%=URL("P_Search",""+Lang(GvFp.Name)+"") %>"><%}else{ %><a href="<%=GvFp.URL%>" target="_blank"><%} %><span><%=Lang(GvFp.Name)%></span></a>
<%GvFp_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int srcR_index=1;
List<Lebi_Page> srcRs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page srcR in srcRs){%>
<a class="menu" href="<%=URL("","",srcR.url)%>" target="<%=srcR.target%>"><span style="color:<%=srcR.NameColor%>"><%=srcR.Name%></span></a> 
<%srcR_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int UFng_index=1;
List<Lebi_FriendLink> UFngs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink UFng in UFngs){%>
            
                <% if (UFng.Logo != "" && UFng.IsPic == 1){ %><a href="<%=UFng.Url%>" target="_blank"><img src="<%=Image(UFng.Logo) %>" alt="<%=UFng.Name%>" /></a><%}else{%><a href="<%=UFng.Url%>" target="_blank"><%=UFng.Name%></a><%} %>     
