...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> Hwdas=Languages();RecordCount=Hwdas.Count;int Hwda_index=1;
foreach (DB.LebiShop.Lebi_Language Hwda in Hwdas){%>
            <dd <%if (Hwda_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=Hwda.id%>,'<%=Hwda.Code%>','<%=Hwda.Path%>');"><%if (Hwda.ImageUrl!=""){%><img src="<%=Image(Hwda.ImageUrl) %>" /><%}%><%=Hwda.Name%></a></dd>
            <%Hwda_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int hRQM_index=1;
List<Lebi_Currency> hRQMs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency hRQM in hRQMs){%>
            <dd <%if (hRQM_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=hRQM.id%>,'<%=hRQM.Code%>',<%=hRQM.ExchangeRate%>,'<%=hRQM.Msige%>','<%=hRQM.DecimalLength%>');"><%=hRQM.Code%></a></dd>
            <%hRQM_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int aIWR_index=1;
List<Lebi_Searchkey> aIWRs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey aIWR in aIWRs){%><%=Lang(aIWR.Name)%><%aIWR_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int aOdh_index=1;
List<Lebi_Searchkey> aOdhs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey aOdh in aOdhs){%>
<%if (aOdh.Type==1){ %><a href="<%=URL("P_Search",""+Lang(aOdh.Name)+"") %>"><%}else{ %><a href="<%=aOdh.URL%>" target="_blank"><%} %><span><%=Lang(aOdh.Name)%></span></a>
<%aOdh_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int EtaM_index=1;
List<Lebi_Page> EtaMs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page EtaM in EtaMs){%>
<a class="menu" href="<%=URL("","",EtaM.url)%>" target="<%=EtaM.target%>"><span style="color:<%=EtaM.NameColor%>"><%=EtaM.Name%></span></a> 
<%EtaM_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int GdSs_index=1;
List<Lebi_FriendLink> GdSss = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink GdSs in GdSss){%>
            
                <% if (GdSs.Logo != "" && GdSs.IsPic == 1){ %><a href="<%=GdSs.Url%>" target="_blank"><img src="<%=Image(GdSs.Logo) %>" alt="<%=GdSs.Name%>" /></a><%}else{%><a href="<%=GdSs.Url%>" target="_blank"><%=GdSs.Name%></a><%} %>     
