...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> fIErs=Languages();RecordCount=fIErs.Count;int fIEr_index=1;
foreach (DB.LebiShop.Lebi_Language fIEr in fIErs){%>
            <dd <%if (fIEr_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=fIEr.id%>,'<%=fIEr.Code%>','<%=fIEr.Path%>');"><%if (fIEr.ImageUrl!=""){%><img src="<%=Image(fIEr.ImageUrl) %>" /><%}%><%=fIEr.Name%></a></dd>
            <%fIEr_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int quvF_index=1;
List<Lebi_Currency> quvFs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency quvF in quvFs){%>
            <dd <%if (quvF_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=quvF.id%>,'<%=quvF.Code%>',<%=quvF.ExchangeRate%>,'<%=quvF.Msige%>','<%=quvF.DecimalLength%>');"><%=quvF.Code%></a></dd>
            <%quvF_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int jPvK_index=1;
List<Lebi_Searchkey> jPvKs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey jPvK in jPvKs){%><%=Lang(jPvK.Name)%><%jPvK_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int wzPd_index=1;
List<Lebi_Searchkey> wzPds = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey wzPd in wzPds){%>
<%if (wzPd.Type==1){ %><a href="<%=URL("P_Search",""+Lang(wzPd.Name)+"") %>"><%}else{ %><a href="<%=wzPd.URL%>" target="_blank"><%} %><span><%=Lang(wzPd.Name)%></span></a>
<%wzPd_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int girK_index=1;
List<Lebi_Page> girKs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page girK in girKs){%>
<a class="menu" href="<%=URL("","",girK.url)%>" target="<%=girK.target%>"><span style="color:<%=girK.NameColor%>"><%=girK.Name%></span></a> 
<%girK_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int TAlV_index=1;
List<Lebi_FriendLink> TAlVs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink TAlV in TAlVs){%>
            
                <% if (TAlV.Logo != "" && TAlV.IsPic == 1){ %><a href="<%=TAlV.Url%>" target="_blank"><img src="<%=Image(TAlV.Logo) %>" alt="<%=TAlV.Name%>" /></a><%}else{%><a href="<%=TAlV.Url%>" target="_blank"><%=TAlV.Name%></a><%} %>     
