...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> NGxcs=Languages();RecordCount=NGxcs.Count;int NGxc_index=1;
foreach (DB.LebiShop.Lebi_Language NGxc in NGxcs){%>
            <dd <%if (NGxc_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=NGxc.id%>,'<%=NGxc.Code%>','<%=NGxc.Path%>');"><%if (NGxc.ImageUrl!=""){%><img src="<%=Image(NGxc.ImageUrl) %>" /><%}%><%=NGxc.Name%></a></dd>
            <%NGxc_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int erpG_index=1;
List<Lebi_Currency> erpGs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency erpG in erpGs){%>
            <dd <%if (erpG_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=erpG.id%>,'<%=erpG.Code%>',<%=erpG.ExchangeRate%>,'<%=erpG.Msige%>','<%=erpG.DecimalLength%>');"><%=erpG.Code%></a></dd>
            <%erpG_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int qols_index=1;
List<Lebi_Searchkey> qolss = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey qols in qolss){%><%=Lang(qols.Name)%><%qols_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int mXRP_index=1;
List<Lebi_Searchkey> mXRPs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey mXRP in mXRPs){%>
<%if (mXRP.Type==1){ %><a href="<%=URL("P_Search",""+Lang(mXRP.Name)+"") %>"><%}else{ %><a href="<%=mXRP.URL%>" target="_blank"><%} %><span><%=Lang(mXRP.Name)%></span></a>
<%mXRP_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int pbEx_index=1;
List<Lebi_Page> pbExs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page pbEx in pbExs){%>
<a class="menu" href="<%=URL("","",pbEx.url)%>" target="<%=pbEx.target%>"><span style="color:<%=pbEx.NameColor%>"><%=pbEx.Name%></span></a> 
<%pbEx_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int EwPC_index=1;
List<Lebi_FriendLink> EwPCs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink EwPC in EwPCs){%>
            
                <% if (EwPC.Logo != "" && EwPC.IsPic == 1){ %><a href="<%=EwPC.Url%>" target="_blank"><img src="<%=Image(EwPC.Logo) %>" alt="<%=EwPC.Name%>" /></a><%}else{%><a href="<%=EwPC.Url%>" target="_blank"><%=EwPC.Name%></a><%} %>     
