...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> ceWQs=Languages();RecordCount=ceWQs.Count;int ceWQ_index=1;
foreach (DB.LebiShop.Lebi_Language ceWQ in ceWQs){%>
            <dd <%if (ceWQ_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=ceWQ.id%>,'<%=ceWQ.Code%>','<%=ceWQ.Path%>');"><%if (ceWQ.ImageUrl!=""){%><img src="<%=Image(ceWQ.ImageUrl) %>" /><%}%><%=ceWQ.Name%></a></dd>
            <%ceWQ_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int DzxX_index=1;
List<Lebi_Currency> DzxXs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency DzxX in DzxXs){%>
            <dd <%if (DzxX_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=DzxX.id%>,'<%=DzxX.Code%>',<%=DzxX.ExchangeRate%>,'<%=DzxX.Msige%>','<%=DzxX.DecimalLength%>');"><%=DzxX.Code%></a></dd>
            <%DzxX_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int BdAv_index=1;
List<Lebi_Searchkey> BdAvs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey BdAv in BdAvs){%><%=Lang(BdAv.Name)%><%BdAv_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Cumt_index=1;
List<Lebi_Searchkey> Cumts = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Cumt in Cumts){%>
<%if (Cumt.Type==1){ %><a href="<%=URL("P_Search",""+Lang(Cumt.Name)+"") %>"><%}else{ %><a href="<%=Cumt.URL%>" target="_blank"><%} %><span><%=Lang(Cumt.Name)%></span></a>
<%Cumt_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int WzvP_index=1;
List<Lebi_Page> WzvPs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page WzvP in WzvPs){%>
<a class="menu" href="<%=URL("","",WzvP.url)%>" target="<%=WzvP.target%>"><span style="color:<%=WzvP.NameColor%>"><%=WzvP.Name%></span></a> 
<%WzvP_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int kwBr_index=1;
List<Lebi_FriendLink> kwBrs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink kwBr in kwBrs){%>
            
                <% if (kwBr.Logo != "" && kwBr.IsPic == 1){ %><a href="<%=kwBr.Url%>" target="_blank"><img src="<%=Image(kwBr.Logo) %>" alt="<%=kwBr.Name%>" /></a><%}else{%><a href="<%=kwBr.Url%>" target="_blank"><%=kwBr.Name%></a><%} %>     
