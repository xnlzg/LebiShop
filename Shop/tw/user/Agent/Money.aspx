...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> aSuvs=Languages();RecordCount=aSuvs.Count;int aSuv_index=1;
foreach (DB.LebiShop.Lebi_Language aSuv in aSuvs){%>
            <dd <%if (aSuv_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=aSuv.id%>,'<%=aSuv.Code%>','<%=aSuv.Path%>');"><%if (aSuv.ImageUrl!=""){%><img src="<%=Image(aSuv.ImageUrl) %>" /><%}%><%=aSuv.Name%></a></dd>
            <%aSuv_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int pFKr_index=1;
List<Lebi_Currency> pFKrs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency pFKr in pFKrs){%>
            <dd <%if (pFKr_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=pFKr.id%>,'<%=pFKr.Code%>',<%=pFKr.ExchangeRate%>,'<%=pFKr.Msige%>','<%=pFKr.DecimalLength%>');"><%=pFKr.Code%></a></dd>
            <%pFKr_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int MMLy_index=1;
List<Lebi_Searchkey> MMLys = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey MMLy in MMLys){%><%=Lang(MMLy.Name)%><%MMLy_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int EoES_index=1;
List<Lebi_Searchkey> EoESs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey EoES in EoESs){%>
<%if (EoES.Type==1){ %><a href="<%=URL("P_Search",""+Lang(EoES.Name)+"") %>"><%}else{ %><a href="<%=EoES.URL%>" target="_blank"><%} %><span><%=Lang(EoES.Name)%></span></a>
<%EoES_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int KubA_index=1;
List<Lebi_Page> KubAs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page KubA in KubAs){%>
<a class="menu" href="<%=URL("","",KubA.url)%>" target="<%=KubA.target%>"><span style="color:<%=KubA.NameColor%>"><%=KubA.Name%></span></a> 
<%KubA_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int bCVM_index=1;
List<Lebi_FriendLink> bCVMs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink bCVM in bCVMs){%>
            
                <% if (bCVM.Logo != "" && bCVM.IsPic == 1){ %><a href="<%=bCVM.Url%>" target="_blank"><img src="<%=Image(bCVM.Logo) %>" alt="<%=bCVM.Name%>" /></a><%}else{%><a href="<%=bCVM.Url%>" target="_blank"><%=bCVM.Name%></a><%} %>     
