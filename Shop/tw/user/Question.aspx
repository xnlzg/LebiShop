...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> pNsos=Languages();RecordCount=pNsos.Count;int pNso_index=1;
foreach (DB.LebiShop.Lebi_Language pNso in pNsos){%>
            <dd <%if (pNso_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=pNso.id%>,'<%=pNso.Code%>','<%=pNso.Path%>');"><%if (pNso.ImageUrl!=""){%><img src="<%=Image(pNso.ImageUrl) %>" /><%}%><%=pNso.Name%></a></dd>
            <%pNso_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int rbXk_index=1;
List<Lebi_Currency> rbXks = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency rbXk in rbXks){%>
            <dd <%if (rbXk_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=rbXk.id%>,'<%=rbXk.Code%>',<%=rbXk.ExchangeRate%>,'<%=rbXk.Msige%>','<%=rbXk.DecimalLength%>');"><%=rbXk.Code%></a></dd>
            <%rbXk_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int gnpg_index=1;
List<Lebi_Searchkey> gnpgs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey gnpg in gnpgs){%><%=Lang(gnpg.Name)%><%gnpg_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int zjPO_index=1;
List<Lebi_Searchkey> zjPOs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey zjPO in zjPOs){%>
<%if (zjPO.Type==1){ %><a href="<%=URL("P_Search",""+Lang(zjPO.Name)+"") %>"><%}else{ %><a href="<%=zjPO.URL%>" target="_blank"><%} %><span><%=Lang(zjPO.Name)%></span></a>
<%zjPO_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int loII_index=1;
List<Lebi_Page> loIIs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page loII in loIIs){%>
<a class="menu" href="<%=URL("","",loII.url)%>" target="<%=loII.target%>"><span style="color:<%=loII.NameColor%>"><%=loII.Name%></span></a> 
<%loII_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int axSj_index=1;
List<Lebi_FriendLink> axSjs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink axSj in axSjs){%>
            
                <% if (axSj.Logo != "" && axSj.IsPic == 1){ %><a href="<%=axSj.Url%>" target="_blank"><img src="<%=Image(axSj.Logo) %>" alt="<%=axSj.Name%>" /></a><%}else{%><a href="<%=axSj.Url%>" target="_blank"><%=axSj.Name%></a><%} %>     
