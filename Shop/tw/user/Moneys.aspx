...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> xHans=Languages();RecordCount=xHans.Count;int xHan_index=1;
foreach (DB.LebiShop.Lebi_Language xHan in xHans){%>
            <dd <%if (xHan_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=xHan.id%>,'<%=xHan.Code%>','<%=xHan.Path%>');"><%if (xHan.ImageUrl!=""){%><img src="<%=Image(xHan.ImageUrl) %>" /><%}%><%=xHan.Name%></a></dd>
            <%xHan_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int QuVx_index=1;
List<Lebi_Currency> QuVxs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency QuVx in QuVxs){%>
            <dd <%if (QuVx_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=QuVx.id%>,'<%=QuVx.Code%>',<%=QuVx.ExchangeRate%>,'<%=QuVx.Msige%>','<%=QuVx.DecimalLength%>');"><%=QuVx.Code%></a></dd>
            <%QuVx_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int fwEq_index=1;
List<Lebi_Searchkey> fwEqs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey fwEq in fwEqs){%><%=Lang(fwEq.Name)%><%fwEq_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int ACEc_index=1;
List<Lebi_Searchkey> ACEcs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey ACEc in ACEcs){%>
<%if (ACEc.Type==1){ %><a href="<%=URL("P_Search",""+Lang(ACEc.Name)+"") %>"><%}else{ %><a href="<%=ACEc.URL%>" target="_blank"><%} %><span><%=Lang(ACEc.Name)%></span></a>
<%ACEc_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int SKQc_index=1;
List<Lebi_Page> SKQcs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page SKQc in SKQcs){%>
<a class="menu" href="<%=URL("","",SKQc.url)%>" target="<%=SKQc.target%>"><span style="color:<%=SKQc.NameColor%>"><%=SKQc.Name%></span></a> 
<%SKQc_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int BoWm_index=1;
List<Lebi_FriendLink> BoWms = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink BoWm in BoWms){%>
            
                <% if (BoWm.Logo != "" && BoWm.IsPic == 1){ %><a href="<%=BoWm.Url%>" target="_blank"><img src="<%=Image(BoWm.Logo) %>" alt="<%=BoWm.Name%>" /></a><%}else{%><a href="<%=BoWm.Url%>" target="_blank"><%=BoWm.Name%></a><%} %>     
