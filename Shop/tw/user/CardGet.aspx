...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> EFXks=Languages();RecordCount=EFXks.Count;int EFXk_index=1;
foreach (DB.LebiShop.Lebi_Language EFXk in EFXks){%>
            <dd <%if (EFXk_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=EFXk.id%>,'<%=EFXk.Code%>','<%=EFXk.Path%>');"><%if (EFXk.ImageUrl!=""){%><img src="<%=Image(EFXk.ImageUrl) %>" /><%}%><%=EFXk.Name%></a></dd>
            <%EFXk_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int uBiS_index=1;
List<Lebi_Currency> uBiSs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency uBiS in uBiSs){%>
            <dd <%if (uBiS_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=uBiS.id%>,'<%=uBiS.Code%>',<%=uBiS.ExchangeRate%>,'<%=uBiS.Msige%>','<%=uBiS.DecimalLength%>');"><%=uBiS.Code%></a></dd>
            <%uBiS_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int KkQY_index=1;
List<Lebi_Searchkey> KkQYs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey KkQY in KkQYs){%><%=Lang(KkQY.Name)%><%KkQY_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int BnQr_index=1;
List<Lebi_Searchkey> BnQrs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey BnQr in BnQrs){%>
<%if (BnQr.Type==1){ %><a href="<%=URL("P_Search",""+Lang(BnQr.Name)+"") %>"><%}else{ %><a href="<%=BnQr.URL%>" target="_blank"><%} %><span><%=Lang(BnQr.Name)%></span></a>
<%BnQr_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int kdjB_index=1;
List<Lebi_Page> kdjBs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page kdjB in kdjBs){%>
<a class="menu" href="<%=URL("","",kdjB.url)%>" target="<%=kdjB.target%>"><span style="color:<%=kdjB.NameColor%>"><%=kdjB.Name%></span></a> 
<%kdjB_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int UYRc_index=1;
List<Lebi_FriendLink> UYRcs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink UYRc in UYRcs){%>
            
                <% if (UYRc.Logo != "" && UYRc.IsPic == 1){ %><a href="<%=UYRc.Url%>" target="_blank"><img src="<%=Image(UYRc.Logo) %>" alt="<%=UYRc.Name%>" /></a><%}else{%><a href="<%=UYRc.Url%>" target="_blank"><%=UYRc.Name%></a><%} %>     
