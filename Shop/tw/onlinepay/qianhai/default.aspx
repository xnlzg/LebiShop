...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> EaIes=Languages();RecordCount=EaIes.Count;int EaIe_index=1;
foreach (DB.LebiShop.Lebi_Language EaIe in EaIes){%>
            <dd <%if (EaIe_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=EaIe.id%>,'<%=EaIe.Code%>','<%=EaIe.Path%>');"><%if (EaIe.ImageUrl!=""){%><img src="<%=Image(EaIe.ImageUrl) %>" /><%}%><%=EaIe.Name%></a></dd>
            <%EaIe_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int FyUH_index=1;
List<Lebi_Currency> FyUHs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency FyUH in FyUHs){%>
            <dd <%if (FyUH_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=FyUH.id%>,'<%=FyUH.Code%>',<%=FyUH.ExchangeRate%>,'<%=FyUH.Msige%>','<%=FyUH.DecimalLength%>');"><%=FyUH.Code%></a></dd>
            <%FyUH_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int vMNh_index=1;
List<Lebi_Searchkey> vMNhs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey vMNh in vMNhs){%><%=Lang(vMNh.Name)%><%vMNh_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int lrrn_index=1;
List<Lebi_Searchkey> lrrns = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey lrrn in lrrns){%>
<%if (lrrn.Type==1){ %><a href="<%=URL("P_Search",""+Lang(lrrn.Name)+"") %>"><%}else{ %><a href="<%=lrrn.URL%>" target="_blank"><%} %><span><%=Lang(lrrn.Name)%></span></a>
<%lrrn_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int hYoR_index=1;
List<Lebi_Page> hYoRs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page hYoR in hYoRs){%>
<a class="menu" href="<%=URL("","",hYoR.url)%>" target="<%=hYoR.target%>"><span style="color:<%=hYoR.NameColor%>"><%=hYoR.Name%></span></a> 
<%hYoR_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int QxRQ_index=1;
List<Lebi_FriendLink> QxRQs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink QxRQ in QxRQs){%>
            
                <% if (QxRQ.Logo != "" && QxRQ.IsPic == 1){ %><a href="<%=QxRQ.Url%>" target="_blank"><img src="<%=Image(QxRQ.Logo) %>" alt="<%=QxRQ.Name%>" /></a><%}else{%><a href="<%=QxRQ.Url%>" target="_blank"><%=QxRQ.Name%></a><%} %>     
