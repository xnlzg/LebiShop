...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> oJOzs=Languages();RecordCount=oJOzs.Count;int oJOz_index=1;
foreach (DB.LebiShop.Lebi_Language oJOz in oJOzs){%>
            <dd <%if (oJOz_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=oJOz.id%>,'<%=oJOz.Code%>','<%=oJOz.Path%>');"><%if (oJOz.ImageUrl!=""){%><img src="<%=Image(oJOz.ImageUrl) %>" /><%}%><%=oJOz.Name%></a></dd>
            <%oJOz_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int QcYt_index=1;
List<Lebi_Currency> QcYts = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency QcYt in QcYts){%>
            <dd <%if (QcYt_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=QcYt.id%>,'<%=QcYt.Code%>',<%=QcYt.ExchangeRate%>,'<%=QcYt.Msige%>','<%=QcYt.DecimalLength%>');"><%=QcYt.Code%></a></dd>
            <%QcYt_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int fOYq_index=1;
List<Lebi_Searchkey> fOYqs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey fOYq in fOYqs){%><%=Lang(fOYq.Name)%><%fOYq_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int laEI_index=1;
List<Lebi_Searchkey> laEIs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey laEI in laEIs){%>
<%if (laEI.Type==1){ %><a href="<%=URL("P_Search",""+Lang(laEI.Name)+"") %>"><%}else{ %><a href="<%=laEI.URL%>" target="_blank"><%} %><span><%=Lang(laEI.Name)%></span></a>
<%laEI_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int tAwD_index=1;
List<Lebi_Page> tAwDs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page tAwD in tAwDs){%>
<a class="menu" href="<%=URL("","",tAwD.url)%>" target="<%=tAwD.target%>"><span style="color:<%=tAwD.NameColor%>"><%=tAwD.Name%></span></a> 
<%tAwD_index++;}%>
...
<div class="categories">
    <%Table="Lebi_Supplier_ProductType";Where="parentid = 0 and Supplier_id = "+id+"";Order="Sort desc,id desc";PageSize=20;pageindex=1;RecordCount=B_Lebi_Supplier_ProductType.Counts(Where);int MyiH_index=1;
List<Lebi_Supplier_ProductType> MyiHs = B_Lebi_Supplier_ProductType.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Supplier_ProductType MyiH in MyiHs){%>
    <dl <%if ((parentcid == 0 && MyiH_index == 1) || parentcid == MyiH.id){ %>class="current" <%} %>>
    <dt class="double">
        <label class="pic"></label>
        <a href="<%=URL("P_ShopProductCategory",""+id+","+MyiH.id+"") %>"><%=Lang(MyiH.Name) %></a>
    </dt>
    <dd>
        <ul>
            <%Table="Lebi_Supplier_ProductType";Where="parentid = "+MyiH.id+" and Supplier_id = "+id+"";Order="Sort desc,id desc";PageSize=20;pageindex=1;RecordCount=B_Lebi_Supplier_ProductType.Counts(Where);int DaQI_index=1;
List<Lebi_Supplier_ProductType> DaQIs = B_Lebi_Supplier_ProductType.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Supplier_ProductType DaQI in DaQIs){%>
            <li class="<%=cid==DaQI.id?"currentlink":"" %>"><a href="<%=URL("P_ShopProductCategory",""+id+","+DaQI.id+"") %>">
                <%=Lang(DaQI.Name) %></a></li>
            <%DaQI_index++;}%>
        </ul>
    </dd>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int vmJz_index=1;
List<Lebi_FriendLink> vmJzs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink vmJz in vmJzs){%>
            
                <% if (vmJz.Logo != "" && vmJz.IsPic == 1){ %><a href="<%=vmJz.Url%>" target="_blank"><img src="<%=Image(vmJz.Logo) %>" alt="<%=vmJz.Name%>" /></a><%}else{%><a href="<%=vmJz.Url%>" target="_blank"><%=vmJz.Name%></a><%} %>     
