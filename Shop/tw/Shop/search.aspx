...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> Ityzs=Languages();RecordCount=Ityzs.Count;int Ityz_index=1;
foreach (DB.LebiShop.Lebi_Language Ityz in Ityzs){%>
            <dd <%if (Ityz_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=Ityz.id%>,'<%=Ityz.Code%>','<%=Ityz.Path%>');"><%if (Ityz.ImageUrl!=""){%><img src="<%=Image(Ityz.ImageUrl) %>" /><%}%><%=Ityz.Name%></a></dd>
            <%Ityz_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int LjeE_index=1;
List<Lebi_Currency> LjeEs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency LjeE in LjeEs){%>
            <dd <%if (LjeE_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=LjeE.id%>,'<%=LjeE.Code%>',<%=LjeE.ExchangeRate%>,'<%=LjeE.Msige%>','<%=LjeE.DecimalLength%>');"><%=LjeE.Code%></a></dd>
            <%LjeE_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int xqaw_index=1;
List<Lebi_Searchkey> xqaws = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey xqaw in xqaws){%><%=Lang(xqaw.Name)%><%xqaw_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int pUba_index=1;
List<Lebi_Searchkey> pUbas = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey pUba in pUbas){%>
<%if (pUba.Type==1){ %><a href="<%=URL("P_Search",""+Lang(pUba.Name)+"") %>"><%}else{ %><a href="<%=pUba.URL%>" target="_blank"><%} %><span><%=Lang(pUba.Name)%></span></a>
<%pUba_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int xgLk_index=1;
List<Lebi_Page> xgLks = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page xgLk in xgLks){%>
<a class="menu" href="<%=URL("","",xgLk.url)%>" target="<%=xgLk.target%>"><span style="color:<%=xgLk.NameColor%>"><%=xgLk.Name%></span></a> 
<%xgLk_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int lBxh_index=1;
List<Lebi_Page> lBxhs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page lBxh in lBxhs){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",lBxh.id,lBxh.url)%>" target="<%=lBxh.target%>" title="<%=lBxh.Name%>"><span style="color:<%=lBxh.NameColor%>"><%=lBxh.Name%></span></a></li>
        <%lBxh_index++;}%>
    </ul>
</div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int OxlY_index=1;
List<Lebi_FriendLink> OxlYs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink OxlY in OxlYs){%>
            
                <% if (OxlY.Logo != "" && OxlY.IsPic == 1){ %><a href="<%=OxlY.Url%>" target="_blank"><img src="<%=Image(OxlY.Logo) %>" alt="<%=OxlY.Name%>" /></a><%}else{%><a href="<%=OxlY.Url%>" target="_blank"><%=OxlY.Name%></a><%} %>     
