...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> pqwns=Languages();RecordCount=pqwns.Count;int pqwn_index=1;
foreach (DB.LebiShop.Lebi_Language pqwn in pqwns){%>
            <dd <%if (pqwn_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=pqwn.id%>,'<%=pqwn.Code%>','<%=pqwn.Path%>');"><%if (pqwn.ImageUrl!=""){%><img src="<%=Image(pqwn.ImageUrl) %>" /><%}%><%=pqwn.Name%></a></dd>
            <%pqwn_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int hLyV_index=1;
List<Lebi_Currency> hLyVs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency hLyV in hLyVs){%>
            <dd <%if (hLyV_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=hLyV.id%>,'<%=hLyV.Code%>',<%=hLyV.ExchangeRate%>,'<%=hLyV.Msige%>','<%=hLyV.DecimalLength%>');"><%=hLyV.Code%></a></dd>
            <%hLyV_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int DRkh_index=1;
List<Lebi_Searchkey> DRkhs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey DRkh in DRkhs){%><%=Lang(DRkh.Name)%><%DRkh_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int tjuu_index=1;
List<Lebi_Searchkey> tjuus = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey tjuu in tjuus){%>
<%if (tjuu.Type==1){ %><a href="<%=URL("P_Search",""+Lang(tjuu.Name)+"") %>"><%}else{ %><a href="<%=tjuu.URL%>" target="_blank"><%} %><span><%=Lang(tjuu.Name)%></span></a>
<%tjuu_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int wfrT_index=1;
List<Lebi_Page> wfrTs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page wfrT in wfrTs){%>
<a class="menu" href="<%=URL("","",wfrT.url)%>" target="<%=wfrT.target%>"><span style="color:<%=wfrT.NameColor%>"><%=wfrT.Name%></span></a> 
<%wfrT_index++;}%>
...
<div class="mc clearfix">
    <ul class="text">
        <%Table="Lebi_Page";Where="Node_id="+Node("News").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Time_Add desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int ruWR_index=1;
List<Lebi_Page> ruWRs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page ruWR in ruWRs){%>
        <li><a href="<%if (ruWR.url == ""){Response.Write(URL("P_NewsDetails",ruWR.id));}else{Response.Write(ruWR.url);} %>" title="<%=ruWR.Name%>"><em><%=ruWR.Time_Add.ToString("MM-dd") %></em><span style="color:<%=ruWR.NameColor%>"><%=ruWR.Name%></span></a></li>
        <%ruWR_index++;}%>
    </ul>
</div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int uPWK_index=1;
List<Lebi_FriendLink> uPWKs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink uPWK in uPWKs){%>
            
                <% if (uPWK.Logo != "" && uPWK.IsPic == 1){ %><a href="<%=uPWK.Url%>" target="_blank"><img src="<%=Image(uPWK.Logo) %>" alt="<%=uPWK.Name%>" /></a><%}else{%><a href="<%=uPWK.Url%>" target="_blank"><%=uPWK.Name%></a><%} %>     
