...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> jwCas=Languages();RecordCount=jwCas.Count;int jwCa_index=1;
foreach (DB.LebiShop.Lebi_Language jwCa in jwCas){%>
            <dd <%if (jwCa_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=jwCa.id%>,'<%=jwCa.Code%>','<%=jwCa.Path%>');"><%if (jwCa.ImageUrl!=""){%><img src="<%=Image(jwCa.ImageUrl) %>" /><%}%><%=jwCa.Name%></a></dd>
            <%jwCa_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int VtDe_index=1;
List<Lebi_Currency> VtDes = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency VtDe in VtDes){%>
            <dd <%if (VtDe_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=VtDe.id%>,'<%=VtDe.Code%>',<%=VtDe.ExchangeRate%>,'<%=VtDe.Msige%>','<%=VtDe.DecimalLength%>');"><%=VtDe.Code%></a></dd>
            <%VtDe_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int CXQf_index=1;
List<Lebi_Searchkey> CXQfs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey CXQf in CXQfs){%><%=Lang(CXQf.Name)%><%CXQf_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int UogW_index=1;
List<Lebi_Searchkey> UogWs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey UogW in UogWs){%>
<%if (UogW.Type==1){ %><a href="<%=URL("P_Search",""+Lang(UogW.Name)+"") %>"><%}else{ %><a href="<%=UogW.URL%>" target="_blank"><%} %><span><%=Lang(UogW.Name)%></span></a>
<%UogW_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int uPpi_index=1;
List<Lebi_Page> uPpis = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page uPpi in uPpis){%>
<a class="menu" href="<%=URL("","",uPpi.url)%>" target="<%=uPpi.target%>"><span style="color:<%=uPpi.NameColor%>"><%=uPpi.Name%></span></a> 
<%uPpi_index++;}%>
...
<div class="mc clearfix">
    <ul class="text">
        <%Table="Lebi_Page";Where="Node_id="+Node("News").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Time_Add desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int gmkQ_index=1;
List<Lebi_Page> gmkQs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page gmkQ in gmkQs){%>
        <li><a href="<%if (gmkQ.url == ""){Response.Write(URL("P_NewsDetails",gmkQ.id));}else{Response.Write(gmkQ.url);} %>" title="<%=gmkQ.Name%>"><em><%=gmkQ.Time_Add.ToString("MM-dd") %></em><span style="color:<%=gmkQ.NameColor%>"><%=gmkQ.Name%></span></a></li>
        <%gmkQ_index++;}%>
    </ul>
</div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int sJwe_index=1;
List<Lebi_FriendLink> sJwes = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink sJwe in sJwes){%>
            
                <% if (sJwe.Logo != "" && sJwe.IsPic == 1){ %><a href="<%=sJwe.Url%>" target="_blank"><img src="<%=Image(sJwe.Logo) %>" alt="<%=sJwe.Name%>" /></a><%}else{%><a href="<%=sJwe.Url%>" target="_blank"><%=sJwe.Name%></a><%} %>     
