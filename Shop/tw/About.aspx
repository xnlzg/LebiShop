...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> sAoys=Languages();RecordCount=sAoys.Count;int sAoy_index=1;
foreach (DB.LebiShop.Lebi_Language sAoy in sAoys){%>
            <dd <%if (sAoy_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=sAoy.id%>,'<%=sAoy.Code%>','<%=sAoy.Path%>');"><%if (sAoy.ImageUrl!=""){%><img src="<%=Image(sAoy.ImageUrl) %>" /><%}%><%=sAoy.Name%></a></dd>
            <%sAoy_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int paND_index=1;
List<Lebi_Currency> paNDs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency paND in paNDs){%>
            <dd <%if (paND_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=paND.id%>,'<%=paND.Code%>',<%=paND.ExchangeRate%>,'<%=paND.Msige%>','<%=paND.DecimalLength%>');"><%=paND.Code%></a></dd>
            <%paND_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int gjoa_index=1;
List<Lebi_Searchkey> gjoas = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey gjoa in gjoas){%><%=Lang(gjoa.Name)%><%gjoa_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int kiIQ_index=1;
List<Lebi_Searchkey> kiIQs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey kiIQ in kiIQs){%>
<%if (kiIQ.Type==1){ %><a href="<%=URL("P_Search",""+Lang(kiIQ.Name)+"") %>"><%}else{ %><a href="<%=kiIQ.URL%>" target="_blank"><%} %><span><%=Lang(kiIQ.Name)%></span></a>
<%kiIQ_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int nBAT_index=1;
List<Lebi_Page> nBATs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page nBAT in nBATs){%>
<a class="menu" href="<%=URL("","",nBAT.url)%>" target="<%=nBAT.target%>"><span style="color:<%=nBAT.NameColor%>"><%=nBAT.Name%></span></a> 
<%nBAT_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int LzKz_index=1;
List<Lebi_FriendLink> LzKzs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink LzKz in LzKzs){%>
            
                <% if (LzKz.Logo != "" && LzKz.IsPic == 1){ %><a href="<%=LzKz.Url%>" target="_blank"><img src="<%=Image(LzKz.Logo) %>" alt="<%=LzKz.Name%>" /></a><%}else{%><a href="<%=LzKz.Url%>" target="_blank"><%=LzKz.Name%></a><%} %>     
