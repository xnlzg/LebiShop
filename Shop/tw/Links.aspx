...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>網站語言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> xodss=Languages();RecordCount=xodss.Count;int xods_index=1;
foreach (DB.LebiShop.Lebi_Language xods in xodss){%>
            <dd <%if (xods_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=xods.id%>,'<%=xods.Code%>','<%=xods.Path%>');"><%if (xods.ImageUrl!=""){%><img src="<%=Image(xods.ImageUrl) %>" /><%}%><%=xods.Name%></a></dd>
            <%xods_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>幣種：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int cGnN_index=1;
List<Lebi_Currency> cGnNs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency cGnN in cGnNs){%>
            <dd <%if (cGnN_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=cGnN.id%>,'<%=cGnN.Code%>',<%=cGnN.ExchangeRate%>,'<%=cGnN.Msige%>','<%=cGnN.DecimalLength%>');"><%=cGnN.Code%></a></dd>
            <%cGnN_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int BljL_index=1;
List<Lebi_Searchkey> BljLs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey BljL in BljLs){%><%=Lang(BljL.Name)%><%BljL_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int bhlh_index=1;
List<Lebi_Searchkey> bhlhs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey bhlh in bhlhs){%>
<%if (bhlh.Type==1){ %><a href="<%=URL("P_Search",""+Lang(bhlh.Name)+"") %>"><%}else{ %><a href="<%=bhlh.URL%>" target="_blank"><%} %><span><%=Lang(bhlh.Name)%></span></a>
<%bhlh_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int YsYp_index=1;
List<Lebi_Page> YsYps = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page YsYp in YsYps){%>
<a class="menu" href="<%=URL("","",YsYp.url)%>" target="<%=YsYp.target%>"><span style="color:<%=YsYp.NameColor%>"><%=YsYp.Name%></span></a> 
<%YsYp_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int BjlH_index=1;
List<Lebi_Page> BjlHs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page BjlH in BjlHs){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",BjlH.id,BjlH.url)%>" target="<%=BjlH.target%>" title="<%=BjlH.Name%>"><span style="color:<%=BjlH.NameColor%>"><%=BjlH.Name%></span></a></li>
        <%BjlH_index++;}%>
    </ul>
</div>
</div>
...
<div class="friendlinklist">
    ...
    <div class="mc clearfix">
        <ul class="image">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and IsPic=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_FriendLink.Counts(Where);int GUym_index=1;
List<Lebi_FriendLink> GUyms = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink GUym in GUyms){%>
            <li>
                <p><a href="<%=GUym.Url%>" target="_blank"><img src="<%=GUym.Logo%>" alt="<%=GUym.Name%>" /></a></p>     
            </li>
            <%GUym_index++;}%>
        </ul>
        <div class="clear"></div>
    ...
    <div class="mc clearfix">
        <ul class="text">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and IsPic=0 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_FriendLink.Counts(Where);int tPNJ_index=1;
List<Lebi_FriendLink> tPNJs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink tPNJ in tPNJs){%>
            <li>
                <a href="<%=tPNJ.Url%>" target="_blank"><%=tPNJ.Name%></a>     
            </li>
            <%tPNJ_index++;}%>
        </ul>
        <div class="clear"></div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int jcVD_index=1;
List<Lebi_FriendLink> jcVDs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink jcVD in jcVDs){%>
            
                <% if (jcVD.Logo != "" && jcVD.IsPic == 1){ %><a href="<%=jcVD.Url%>" target="_blank"><img src="<%=Image(jcVD.Logo) %>" alt="<%=jcVD.Name%>" /></a><%}else{%><a href="<%=jcVD.Url%>" target="_blank"><%=jcVD.Name%></a><%} %>     
