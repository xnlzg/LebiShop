...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> AwGUs=Languages();RecordCount=AwGUs.Count;int AwGU_index=1;
foreach (DB.LebiShop.Lebi_Language AwGU in AwGUs){%>
            <dd <%if (AwGU_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=AwGU.id%>,'<%=AwGU.Code%>','<%=AwGU.Path%>');"><%if (AwGU.ImageUrl!=""){%><img src="<%=Image(AwGU.ImageUrl) %>" /><%}%><%=AwGU.Name%></a></dd>
            <%AwGU_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int tHvv_index=1;
List<Lebi_Currency> tHvvs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency tHvv in tHvvs){%>
            <dd <%if (tHvv_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=tHvv.id%>,'<%=tHvv.Code%>',<%=tHvv.ExchangeRate%>,'<%=tHvv.Msige%>','<%=tHvv.DecimalLength%>');"><%=tHvv.Code%></a></dd>
            <%tHvv_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int CliY_index=1;
List<Lebi_Searchkey> CliYs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey CliY in CliYs){%><%=Lang(CliY.Name)%><%CliY_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int enYt_index=1;
List<Lebi_Searchkey> enYts = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey enYt in enYts){%>
<%if (enYt.Type==1){ %><a href="<%=URL("P_Search",""+Lang(enYt.Name)+"") %>"><%}else{ %><a href="<%=enYt.URL%>" target="_blank"><%} %><span><%=Lang(enYt.Name)%></span></a>
<%enYt_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int OUay_index=1;
List<Lebi_Page> OUays = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page OUay in OUays){%>
<a class="menu" href="<%=URL("","",OUay.url)%>" target="<%=OUay.target%>"><span style="color:<%=OUay.NameColor%>"><%=OUay.Name%></span></a> 
<%OUay_index++;}%>
...
<div class="mc clearfix">
    <ul class="text">
        <%Table="Lebi_Page";Where="Node_id="+Node("News").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Time_Add desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int FMjN_index=1;
List<Lebi_Page> FMjNs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page FMjN in FMjNs){%>
        <li><a href="<%if (FMjN.url == ""){Response.Write(URL("P_NewsDetails",FMjN.id));}else{Response.Write(FMjN.url);} %>" title="<%=FMjN.Name%>"><em><%=FMjN.Time_Add.ToString("MM-dd") %></em><span style="color:<%=FMjN.NameColor%>"><%=FMjN.Name%></span></a></li>
        <%FMjN_index++;}%>
    </ul>
</div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int Lylc_index=1;
List<Lebi_FriendLink> Lylcs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink Lylc in Lylcs){%>
            
                <% if (Lylc.Logo != "" && Lylc.IsPic == 1){ %><a href="<%=Lylc.Url%>" target="_blank"><img src="<%=Image(Lylc.Logo) %>" alt="<%=Lylc.Name%>" /></a><%}else{%><a href="<%=Lylc.Url%>" target="_blank"><%=Lylc.Name%></a><%} %>     
