...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> zyvBs=Languages();RecordCount=zyvBs.Count;int zyvB_index=1;
foreach (DB.LebiShop.Lebi_Language zyvB in zyvBs){%>
            <dd <%if (zyvB_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=zyvB.id%>,'<%=zyvB.Code%>','<%=zyvB.Path%>');"><%if (zyvB.ImageUrl!=""){%><img src="<%=Image(zyvB.ImageUrl) %>" /><%}%><%=zyvB.Name%></a></dd>
            <%zyvB_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int KOTM_index=1;
List<Lebi_Currency> KOTMs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency KOTM in KOTMs){%>
            <dd <%if (KOTM_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=KOTM.id%>,'<%=KOTM.Code%>',<%=KOTM.ExchangeRate%>,'<%=KOTM.Msige%>','<%=KOTM.DecimalLength%>');"><%=KOTM.Code%></a></dd>
            <%KOTM_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int OlVr_index=1;
List<Lebi_Searchkey> OlVrs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey OlVr in OlVrs){%><%=Lang(OlVr.Name)%><%OlVr_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int MIaC_index=1;
List<Lebi_Searchkey> MIaCs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey MIaC in MIaCs){%>
<%if (MIaC.Type==1){ %><a href="<%=URL("P_Search",""+Lang(MIaC.Name)+"") %>"><%}else{ %><a href="<%=MIaC.URL%>" target="_blank"><%} %><span><%=Lang(MIaC.Name)%></span></a>
<%MIaC_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int rLOC_index=1;
List<Lebi_Page> rLOCs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page rLOC in rLOCs){%>
<a class="menu" href="<%=URL("","",rLOC.url)%>" target="<%=rLOC.target%>"><span style="color:<%=rLOC.NameColor%>"><%=rLOC.Name%></span></a> 
<%rLOC_index++;}%>
...
<div class="ind-tabs">
	<div class="hd">
    	<ul>
        <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and IsRecommend=1";Order="Sort desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int Yddn_index=1;
List<Lebi_Brand> Yddns = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand Yddn in Yddns){%>
        	<li><%=Lang(Yddn.Name) %></li>
        <%Yddn_index++;}%>
        </ul>
    </div>
    <div class="bd">
      <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and IsRecommend=1";Order="Sort desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int bSTg_index=1;
List<Lebi_Brand> bSTgs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand bSTg in bSTgs){%>
   	  <div class="lh">
        	<div class="pic"><a href="<%=URL("P_Brand",bSTg.id)%>"><img src="<%=Image(bSTg.ImageUrl) %>" alt="<%=Lang(bSTg.Name) %>" title="<%=Lang(bSTg.Name) %>" width="230" height="230" /></a></div>
            <div class="info">
            	<div class="desc"><%=Lang(bSTg.Description)%></div>
                <a href="<%=URL("P_Brand",bSTg.id)%>" class="read-more"><%=Lang(bSTg.Name) %> 商品列表 >></a>
            </div>
      </div>
      <%bSTg_index++;}%>
    </div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int NsbW_index=1;
List<Lebi_FriendLink> NsbWs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink NsbW in NsbWs){%>
            
                <% if (NsbW.Logo != "" && NsbW.IsPic == 1){ %><a href="<%=NsbW.Url%>" target="_blank"><img src="<%=Image(NsbW.Logo) %>" alt="<%=NsbW.Name%>" /></a><%}else{%><a href="<%=NsbW.Url%>" target="_blank"><%=NsbW.Name%></a><%} %>     
