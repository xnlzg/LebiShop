...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> CNKFs=Languages();RecordCount=CNKFs.Count;int CNKF_index=1;
foreach (DB.LebiShop.Lebi_Language CNKF in CNKFs){%>
            <dd <%if (CNKF_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=CNKF.id%>,'<%=CNKF.Code%>','<%=CNKF.Path%>');"><%if (CNKF.ImageUrl!=""){%><img src="<%=Image(CNKF.ImageUrl) %>" /><%}%><%=CNKF.Name%></a></dd>
            <%CNKF_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int ASxo_index=1;
List<Lebi_Currency> ASxos = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency ASxo in ASxos){%>
            <dd <%if (ASxo_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=ASxo.id%>,'<%=ASxo.Code%>',<%=ASxo.ExchangeRate%>,'<%=ASxo.Msige%>','<%=ASxo.DecimalLength%>');"><%=ASxo.Code%></a></dd>
            <%ASxo_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int nXGL_index=1;
List<Lebi_Searchkey> nXGLs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey nXGL in nXGLs){%><%=Lang(nXGL.Name)%><%nXGL_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int dxzU_index=1;
List<Lebi_Searchkey> dxzUs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey dxzU in dxzUs){%>
<%if (dxzU.Type==1){ %><a href="<%=URL("P_Search",""+Lang(dxzU.Name)+"") %>"><%}else{ %><a href="<%=dxzU.URL%>" target="_blank"><%} %><span><%=Lang(dxzU.Name)%></span></a>
<%dxzU_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int ywmk_index=1;
List<Lebi_Page> ywmks = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page ywmk in ywmks){%>
<a class="menu" href="<%=URL("","",ywmk.url)%>" target="<%=ywmk.target%>"><span style="color:<%=ywmk.NameColor%>"><%=ywmk.Name%></span></a> 
<%ywmk_index++;}%>
...
<ul class="text">
    <%Table="Lebi_Brand";Where="Type_id_BrandStatus = 452 and IsRecommend=1";Order="Sort desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Brand.Counts(Where);int qWjG_index=1;
List<Lebi_Brand> qWjGs = B_Lebi_Brand.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Brand qWjG in qWjGs){%>
    <li><a href="<%=URL("P_Brand",qWjG.id)%>" title="<%=Lang(qWjG.Name) %>"><%=Lang(qWjG.Name) %></a></li>
    <%qWjG_index++;}%>
</ul>
</div>
</div>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int btxM_index=1;
List<Lebi_Page> btxMs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page btxM in btxMs){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",btxM.id,btxM.url)%>" target="<%=btxM.target%>" title="<%=btxM.Name%>"><span style="color:<%=btxM.NameColor%>"><%=btxM.Name%></span></a></li>
        <%btxM_index++;}%>
    </ul>
</div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int wvcr_index=1;
List<Lebi_FriendLink> wvcrs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink wvcr in wvcrs){%>
            
                <% if (wvcr.Logo != "" && wvcr.IsPic == 1){ %><a href="<%=wvcr.Url%>" target="_blank"><img src="<%=Image(wvcr.Logo) %>" alt="<%=wvcr.Name%>" /></a><%}else{%><a href="<%=wvcr.Url%>" target="_blank"><%=wvcr.Name%></a><%} %>     
