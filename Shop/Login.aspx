...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> eAuRs=Languages();RecordCount=eAuRs.Count;int eAuR_index=1;
foreach (DB.LebiShop.Lebi_Language eAuR in eAuRs){%>
            <dd <%if (eAuR_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=eAuR.id%>,'<%=eAuR.Code%>','<%=eAuR.Path%>');"><%if (eAuR.ImageUrl!=""){%><img src="<%=Image(eAuR.ImageUrl) %>" /><%}%><%=eAuR.Name%></a></dd>
            <%eAuR_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int qdfz_index=1;
List<Lebi_Currency> qdfzs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency qdfz in qdfzs){%>
            <dd <%if (qdfz_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=qdfz.id%>,'<%=qdfz.Code%>',<%=qdfz.ExchangeRate%>,'<%=qdfz.Msige%>','<%=qdfz.DecimalLength%>');"><%=qdfz.Code%></a></dd>
            <%qdfz_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int oHEg_index=1;
List<Lebi_Searchkey> oHEgs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey oHEg in oHEgs){%><%=Lang(oHEg.Name)%><%oHEg_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int xpeb_index=1;
List<Lebi_Searchkey> xpebs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey xpeb in xpebs){%>
<%if (xpeb.Type==1){ %><a href="<%=URL("P_Search",""+Lang(xpeb.Name)+"") %>"><%}else{ %><a href="<%=xpeb.URL%>" target="_blank"><%} %><span><%=Lang(xpeb.Name)%></span></a>
<%xpeb_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int xBoj_index=1;
List<Lebi_Page> xBojs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page xBoj in xBojs){%>
<a class="menu" href="<%=URL("","",xBoj.url)%>" target="<%=xBoj.target%>"><span style="color:<%=xBoj.NameColor%>"><%=xBoj.Name%></span></a> 
<%xBoj_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int Xlkd_index=1;
List<Lebi_FriendLink> Xlkds = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink Xlkd in Xlkds){%>
            
                <% if (Xlkd.Logo != "" && Xlkd.IsPic == 1){ %><a href="<%=Xlkd.Url%>" target="_blank"><img src="<%=Image(Xlkd.Logo) %>" alt="<%=Xlkd.Name%>" /></a><%}else{%><a href="<%=Xlkd.Url%>" target="_blank"><%=Xlkd.Name%></a><%} %>     
