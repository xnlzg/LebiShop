...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> nEqos=Languages();RecordCount=nEqos.Count;int nEqo_index=1;
foreach (DB.LebiShop.Lebi_Language nEqo in nEqos){%>
            <dd <%if (nEqo_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=nEqo.id%>,'<%=nEqo.Code%>','<%=nEqo.Path%>');"><%if (nEqo.ImageUrl!=""){%><img src="<%=Image(nEqo.ImageUrl) %>" /><%}%><%=nEqo.Name%></a></dd>
            <%nEqo_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int NGKA_index=1;
List<Lebi_Currency> NGKAs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency NGKA in NGKAs){%>
            <dd <%if (NGKA_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=NGKA.id%>,'<%=NGKA.Code%>',<%=NGKA.ExchangeRate%>,'<%=NGKA.Msige%>','<%=NGKA.DecimalLength%>');"><%=NGKA.Code%></a></dd>
            <%NGKA_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Jfiy_index=1;
List<Lebi_Searchkey> Jfiys = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Jfiy in Jfiys){%><%=Lang(Jfiy.Name)%><%Jfiy_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int DrVs_index=1;
List<Lebi_Searchkey> DrVss = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey DrVs in DrVss){%>
<%if (DrVs.Type==1){ %><a href="<%=URL("P_Search",""+Lang(DrVs.Name)+"") %>"><%}else{ %><a href="<%=DrVs.URL%>" target="_blank"><%} %><span><%=Lang(DrVs.Name)%></span></a>
<%DrVs_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int aUwi_index=1;
List<Lebi_Page> aUwis = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page aUwi in aUwis){%>
<a class="menu" href="<%=URL("","",aUwi.url)%>" target="<%=aUwi.target%>"><span style="color:<%=aUwi.NameColor%>"><%=aUwi.Name%></span></a> 
<%aUwi_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int AWxL_index=1;
List<Lebi_FriendLink> AWxLs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink AWxL in AWxLs){%>
            
                <% if (AWxL.Logo != "" && AWxL.IsPic == 1){ %><a href="<%=AWxL.Url%>" target="_blank"><img src="<%=Image(AWxL.Logo) %>" alt="<%=AWxL.Name%>" /></a><%}else{%><a href="<%=AWxL.Url%>" target="_blank"><%=AWxL.Name%></a><%} %>     
