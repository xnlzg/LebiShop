...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> rSEPs=Languages();RecordCount=rSEPs.Count;int rSEP_index=1;
foreach (DB.LebiShop.Lebi_Language rSEP in rSEPs){%>
            <dd <%if (rSEP_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=rSEP.id%>,'<%=rSEP.Code%>','<%=rSEP.Path%>');"><%if (rSEP.ImageUrl!=""){%><img src="<%=Image(rSEP.ImageUrl) %>" /><%}%><%=rSEP.Name%></a></dd>
            <%rSEP_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int CLFo_index=1;
List<Lebi_Currency> CLFos = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency CLFo in CLFos){%>
            <dd <%if (CLFo_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=CLFo.id%>,'<%=CLFo.Code%>',<%=CLFo.ExchangeRate%>,'<%=CLFo.Msige%>','<%=CLFo.DecimalLength%>');"><%=CLFo.Code%></a></dd>
            <%CLFo_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int wyxy_index=1;
List<Lebi_Searchkey> wyxys = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey wyxy in wyxys){%><%=Lang(wyxy.Name)%><%wyxy_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int LfGx_index=1;
List<Lebi_Searchkey> LfGxs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey LfGx in LfGxs){%>
<%if (LfGx.Type==1){ %><a href="<%=URL("P_Search",""+Lang(LfGx.Name)+"") %>"><%}else{ %><a href="<%=LfGx.URL%>" target="_blank"><%} %><span><%=Lang(LfGx.Name)%></span></a>
<%LfGx_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int KOsT_index=1;
List<Lebi_Page> KOsTs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page KOsT in KOsTs){%>
<a class="menu" href="<%=URL("","",KOsT.url)%>" target="<%=KOsT.target%>"><span style="color:<%=KOsT.NameColor%>"><%=KOsT.Name%></span></a> 
<%KOsT_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int avHb_index=1;
List<Lebi_FriendLink> avHbs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink avHb in avHbs){%>
            
                <% if (avHb.Logo != "" && avHb.IsPic == 1){ %><a href="<%=avHb.Url%>" target="_blank"><img src="<%=Image(avHb.Logo) %>" alt="<%=avHb.Name%>" /></a><%}else{%><a href="<%=avHb.Url%>" target="_blank"><%=avHb.Name%></a><%} %>     
