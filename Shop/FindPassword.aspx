...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> wlVAs=Languages();RecordCount=wlVAs.Count;int wlVA_index=1;
foreach (DB.LebiShop.Lebi_Language wlVA in wlVAs){%>
            <dd <%if (wlVA_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=wlVA.id%>,'<%=wlVA.Code%>','<%=wlVA.Path%>');"><%if (wlVA.ImageUrl!=""){%><img src="<%=Image(wlVA.ImageUrl) %>" /><%}%><%=wlVA.Name%></a></dd>
            <%wlVA_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int AMXH_index=1;
List<Lebi_Currency> AMXHs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency AMXH in AMXHs){%>
            <dd <%if (AMXH_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=AMXH.id%>,'<%=AMXH.Code%>',<%=AMXH.ExchangeRate%>,'<%=AMXH.Msige%>','<%=AMXH.DecimalLength%>');"><%=AMXH.Code%></a></dd>
            <%AMXH_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int oksh_index=1;
List<Lebi_Searchkey> okshs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey oksh in okshs){%><%=Lang(oksh.Name)%><%oksh_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int UlwJ_index=1;
List<Lebi_Searchkey> UlwJs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey UlwJ in UlwJs){%>
<%if (UlwJ.Type==1){ %><a href="<%=URL("P_Search",""+Lang(UlwJ.Name)+"") %>"><%}else{ %><a href="<%=UlwJ.URL%>" target="_blank"><%} %><span><%=Lang(UlwJ.Name)%></span></a>
<%UlwJ_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int yIhP_index=1;
List<Lebi_Page> yIhPs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page yIhP in yIhPs){%>
<a class="menu" href="<%=URL("","",yIhP.url)%>" target="<%=yIhP.target%>"><span style="color:<%=yIhP.NameColor%>"><%=yIhP.Name%></span></a> 
<%yIhP_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int MALl_index=1;
List<Lebi_FriendLink> MALls = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink MALl in MALls){%>
            
                <% if (MALl.Logo != "" && MALl.IsPic == 1){ %><a href="<%=MALl.Url%>" target="_blank"><img src="<%=Image(MALl.Logo) %>" alt="<%=MALl.Name%>" /></a><%}else{%><a href="<%=MALl.Url%>" target="_blank"><%=MALl.Name%></a><%} %>     
