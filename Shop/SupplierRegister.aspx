...
<% LoadPage("wangdayu523@163.com_37",1,"CN","P_SupplierRegister"); %>
...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> qEVfs=Languages();RecordCount=qEVfs.Count;int qEVf_index=1;
foreach (DB.LebiShop.Lebi_Language qEVf in qEVfs){%>
            <dd <%if (qEVf_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=qEVf.id%>,'<%=qEVf.Code%>','<%=qEVf.Path%>');"><%if (qEVf.ImageUrl!=""){%><img src="<%=Image(qEVf.ImageUrl) %>" /><%}%><%=qEVf.Name%></a></dd>
            <%qEVf_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int yPEb_index=1;
List<Lebi_Currency> yPEbs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency yPEb in yPEbs){%>
            <dd <%if (yPEb_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=yPEb.id%>,'<%=yPEb.Code%>',<%=yPEb.ExchangeRate%>,'<%=yPEb.Msige%>','<%=yPEb.DecimalLength%>');"><%=yPEb.Code%></a></dd>
            <%yPEb_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int rMBE_index=1;
List<Lebi_Searchkey> rMBEs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey rMBE in rMBEs){%><%=Lang(rMBE.Name)%><%rMBE_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int taAt_index=1;
List<Lebi_Searchkey> taAts = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey taAt in taAts){%>
<%if (taAt.Type==1){ %><a href="<%=URL("P_Search",""+Lang(taAt.Name)+"") %>"><%}else{ %><a href="<%=taAt.URL%>" target="_blank"><%} %><span><%=Lang(taAt.Name)%></span></a>
<%taAt_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int MpBz_index=1;
List<Lebi_Page> MpBzs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page MpBz in MpBzs){%>
<a class="menu" href="<%=URL("","",MpBz.url)%>" target="<%=MpBz.target%>"><span style="color:<%=MpBz.NameColor%>"><%=MpBz.Name%></span></a> 
<%MpBz_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int LaIa_index=1;
List<Lebi_FriendLink> LaIas = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink LaIa in LaIas){%>
            
                <% if (LaIa.Logo != "" && LaIa.IsPic == 1){ %><a href="<%=LaIa.Url%>" target="_blank"><img src="<%=Image(LaIa.Logo) %>" alt="<%=LaIa.Name%>" /></a><%}else{%><a href="<%=LaIa.Url%>" target="_blank"><%=LaIa.Name%></a><%} %>     
