...
<body class="fix-sidebar fix-header card-no-border">
    ...
    <h3 class="text-themecolor m-b-0 m-t-0"><%=Tag("会员积分")%></h3>
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="<%=site.AdminPath %>/Ajax/ajax_admin.aspx?__Action=MenuJump&pid=0"><%=Tag("管理首页")%></a></li>
        <li class="breadcrumb-item"><a href="<%=site.AdminPath %>/user/default.aspx"><%=Tag("会员管理")%></a></li>
        <%if (user.UserName != ""){ %>
        <li class="breadcrumb-item"><a href="<%=site.AdminPath %>/user/UserPoint.aspx"><%=Tag("会员积分")%></a></li>
        <li class="breadcrumb-item active"><%=user.UserName%></li>
        <%}else{%>
        <li class="breadcrumb-item active"><%=Tag("会员积分")%></li>
        <%}%>
    </ol>
...
<script>
    ...
    <script>
        ...
        function EditObj(User_Name,id) {
            ...
            $("#EditModal").modal("show");
            $.ajax({
                url: "userpoint_edit_window.aspx",
                type: "GET",
                cache: false,
