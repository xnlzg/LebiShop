...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> GcXhs=Languages();RecordCount=GcXhs.Count;int GcXh_index=1;
foreach (DB.LebiShop.Lebi_Language GcXh in GcXhs){%>
            <dd <%if (GcXh_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=GcXh.id%>,'<%=GcXh.Code%>','<%=GcXh.Path%>');"><%if (GcXh.ImageUrl!=""){%><img src="<%=Image(GcXh.ImageUrl) %>" /><%}%><%=GcXh.Name%></a></dd>
            <%GcXh_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int wSNk_index=1;
List<Lebi_Currency> wSNks = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency wSNk in wSNks){%>
            <dd <%if (wSNk_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=wSNk.id%>,'<%=wSNk.Code%>',<%=wSNk.ExchangeRate%>,'<%=wSNk.Msige%>','<%=wSNk.DecimalLength%>');"><%=wSNk.Code%></a></dd>
            <%wSNk_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int XuHy_index=1;
List<Lebi_Searchkey> XuHys = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey XuHy in XuHys){%><%=Lang(XuHy.Name)%><%XuHy_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int HTJG_index=1;
List<Lebi_Searchkey> HTJGs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey HTJG in HTJGs){%>
<%if (HTJG.Type==1){ %><a href="<%=URL("P_Search",""+Lang(HTJG.Name)+"") %>"><%}else{ %><a href="<%=HTJG.URL%>" target="_blank"><%} %><span><%=Lang(HTJG.Name)%></span></a>
<%HTJG_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int Dlht_index=1;
List<Lebi_Page> Dlhts = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page Dlht in Dlhts){%>
<a class="menu" href="<%=URL("","",Dlht.url)%>" target="<%=Dlht.target%>"><span style="color:<%=Dlht.NameColor%>"><%=Dlht.Name%></span></a> 
<%Dlht_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int UoVD_index=1;
List<Lebi_FriendLink> UoVDs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink UoVD in UoVDs){%>
            
                <% if (UoVD.Logo != "" && UoVD.IsPic == 1){ %><a href="<%=UoVD.Url%>" target="_blank"><img src="<%=Image(UoVD.Logo) %>" alt="<%=UoVD.Name%>" /></a><%}else{%><a href="<%=UoVD.Url%>" target="_blank"><%=UoVD.Name%></a><%} %>     
