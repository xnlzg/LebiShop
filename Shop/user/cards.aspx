...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> mUjrs=Languages();RecordCount=mUjrs.Count;int mUjr_index=1;
foreach (DB.LebiShop.Lebi_Language mUjr in mUjrs){%>
            <dd <%if (mUjr_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=mUjr.id%>,'<%=mUjr.Code%>','<%=mUjr.Path%>');"><%if (mUjr.ImageUrl!=""){%><img src="<%=Image(mUjr.ImageUrl) %>" /><%}%><%=mUjr.Name%></a></dd>
            <%mUjr_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int BCUC_index=1;
List<Lebi_Currency> BCUCs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency BCUC in BCUCs){%>
            <dd <%if (BCUC_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=BCUC.id%>,'<%=BCUC.Code%>',<%=BCUC.ExchangeRate%>,'<%=BCUC.Msige%>','<%=BCUC.DecimalLength%>');"><%=BCUC.Code%></a></dd>
            <%BCUC_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int rBJh_index=1;
List<Lebi_Searchkey> rBJhs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey rBJh in rBJhs){%><%=Lang(rBJh.Name)%><%rBJh_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int cBlA_index=1;
List<Lebi_Searchkey> cBlAs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey cBlA in cBlAs){%>
<%if (cBlA.Type==1){ %><a href="<%=URL("P_Search",""+Lang(cBlA.Name)+"") %>"><%}else{ %><a href="<%=cBlA.URL%>" target="_blank"><%} %><span><%=Lang(cBlA.Name)%></span></a>
<%cBlA_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int YPeK_index=1;
List<Lebi_Page> YPeKs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page YPeK in YPeKs){%>
<a class="menu" href="<%=URL("","",YPeK.url)%>" target="<%=YPeK.target%>"><span style="color:<%=YPeK.NameColor%>"><%=YPeK.Name%></span></a> 
<%YPeK_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int RXUb_index=1;
List<Lebi_FriendLink> RXUbs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink RXUb in RXUbs){%>
            
                <% if (RXUb.Logo != "" && RXUb.IsPic == 1){ %><a href="<%=RXUb.Url%>" target="_blank"><img src="<%=Image(RXUb.Logo) %>" alt="<%=RXUb.Name%>" /></a><%}else{%><a href="<%=RXUb.Url%>" target="_blank"><%=RXUb.Name%></a><%} %>     
