...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> DtPCs=Languages();RecordCount=DtPCs.Count;int DtPC_index=1;
foreach (DB.LebiShop.Lebi_Language DtPC in DtPCs){%>
            <dd <%if (DtPC_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=DtPC.id%>,'<%=DtPC.Code%>','<%=DtPC.Path%>');"><%if (DtPC.ImageUrl!=""){%><img src="<%=Image(DtPC.ImageUrl) %>" /><%}%><%=DtPC.Name%></a></dd>
            <%DtPC_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int GmGs_index=1;
List<Lebi_Currency> GmGss = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency GmGs in GmGss){%>
            <dd <%if (GmGs_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=GmGs.id%>,'<%=GmGs.Code%>',<%=GmGs.ExchangeRate%>,'<%=GmGs.Msige%>','<%=GmGs.DecimalLength%>');"><%=GmGs.Code%></a></dd>
            <%GmGs_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int IAHo_index=1;
List<Lebi_Searchkey> IAHos = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey IAHo in IAHos){%><%=Lang(IAHo.Name)%><%IAHo_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int vMNo_index=1;
List<Lebi_Searchkey> vMNos = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey vMNo in vMNos){%>
<%if (vMNo.Type==1){ %><a href="<%=URL("P_Search",""+Lang(vMNo.Name)+"") %>"><%}else{ %><a href="<%=vMNo.URL%>" target="_blank"><%} %><span><%=Lang(vMNo.Name)%></span></a>
<%vMNo_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int zbBz_index=1;
List<Lebi_Page> zbBzs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page zbBz in zbBzs){%>
<a class="menu" href="<%=URL("","",zbBz.url)%>" target="<%=zbBz.target%>"><span style="color:<%=zbBz.NameColor%>"><%=zbBz.Name%></span></a> 
<%zbBz_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int goUT_index=1;
List<Lebi_FriendLink> goUTs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink goUT in goUTs){%>
            
                <% if (goUT.Logo != "" && goUT.IsPic == 1){ %><a href="<%=goUT.Url%>" target="_blank"><img src="<%=Image(goUT.Logo) %>" alt="<%=goUT.Name%>" /></a><%}else{%><a href="<%=goUT.Url%>" target="_blank"><%=goUT.Name%></a><%} %>     
