...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> SmJEs=Languages();RecordCount=SmJEs.Count;int SmJE_index=1;
foreach (DB.LebiShop.Lebi_Language SmJE in SmJEs){%>
            <dd <%if (SmJE_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=SmJE.id%>,'<%=SmJE.Code%>','<%=SmJE.Path%>');"><%if (SmJE.ImageUrl!=""){%><img src="<%=Image(SmJE.ImageUrl) %>" /><%}%><%=SmJE.Name%></a></dd>
            <%SmJE_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int LpqT_index=1;
List<Lebi_Currency> LpqTs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency LpqT in LpqTs){%>
            <dd <%if (LpqT_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=LpqT.id%>,'<%=LpqT.Code%>',<%=LpqT.ExchangeRate%>,'<%=LpqT.Msige%>','<%=LpqT.DecimalLength%>');"><%=LpqT.Code%></a></dd>
            <%LpqT_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Abvp_index=1;
List<Lebi_Searchkey> Abvps = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Abvp in Abvps){%><%=Lang(Abvp.Name)%><%Abvp_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int orxn_index=1;
List<Lebi_Searchkey> orxns = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey orxn in orxns){%>
<%if (orxn.Type==1){ %><a href="<%=URL("P_Search",""+Lang(orxn.Name)+"") %>"><%}else{ %><a href="<%=orxn.URL%>" target="_blank"><%} %><span><%=Lang(orxn.Name)%></span></a>
<%orxn_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int LSzR_index=1;
List<Lebi_Page> LSzRs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page LSzR in LSzRs){%>
<a class="menu" href="<%=URL("","",LSzR.url)%>" target="<%=LSzR.target%>"><span style="color:<%=LSzR.NameColor%>"><%=LSzR.Name%></span></a> 
<%LSzR_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int tImA_index=1;
List<Lebi_FriendLink> tImAs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink tImA in tImAs){%>
            
                <% if (tImA.Logo != "" && tImA.IsPic == 1){ %><a href="<%=tImA.Url%>" target="_blank"><img src="<%=Image(tImA.Logo) %>" alt="<%=tImA.Name%>" /></a><%}else{%><a href="<%=tImA.Url%>" target="_blank"><%=tImA.Name%></a><%} %>     
