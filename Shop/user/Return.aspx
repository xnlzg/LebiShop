...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> YFoks=Languages();RecordCount=YFoks.Count;int YFok_index=1;
foreach (DB.LebiShop.Lebi_Language YFok in YFoks){%>
            <dd <%if (YFok_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=YFok.id%>,'<%=YFok.Code%>','<%=YFok.Path%>');"><%if (YFok.ImageUrl!=""){%><img src="<%=Image(YFok.ImageUrl) %>" /><%}%><%=YFok.Name%></a></dd>
            <%YFok_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int wTWT_index=1;
List<Lebi_Currency> wTWTs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency wTWT in wTWTs){%>
            <dd <%if (wTWT_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=wTWT.id%>,'<%=wTWT.Code%>',<%=wTWT.ExchangeRate%>,'<%=wTWT.Msige%>','<%=wTWT.DecimalLength%>');"><%=wTWT.Code%></a></dd>
            <%wTWT_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int tozk_index=1;
List<Lebi_Searchkey> tozks = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey tozk in tozks){%><%=Lang(tozk.Name)%><%tozk_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int SkEW_index=1;
List<Lebi_Searchkey> SkEWs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey SkEW in SkEWs){%>
<%if (SkEW.Type==1){ %><a href="<%=URL("P_Search",""+Lang(SkEW.Name)+"") %>"><%}else{ %><a href="<%=SkEW.URL%>" target="_blank"><%} %><span><%=Lang(SkEW.Name)%></span></a>
<%SkEW_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int eygp_index=1;
List<Lebi_Page> eygps = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page eygp in eygps){%>
<a class="menu" href="<%=URL("","",eygp.url)%>" target="<%=eygp.target%>"><span style="color:<%=eygp.NameColor%>"><%=eygp.Name%></span></a> 
<%eygp_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int NFAQ_index=1;
List<Lebi_FriendLink> NFAQs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink NFAQ in NFAQs){%>
            
                <% if (NFAQ.Logo != "" && NFAQ.IsPic == 1){ %><a href="<%=NFAQ.Url%>" target="_blank"><img src="<%=Image(NFAQ.Logo) %>" alt="<%=NFAQ.Name%>" /></a><%}else{%><a href="<%=NFAQ.Url%>" target="_blank"><%=NFAQ.Name%></a><%} %>     
