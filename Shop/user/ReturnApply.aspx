...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> WXxgs=Languages();RecordCount=WXxgs.Count;int WXxg_index=1;
foreach (DB.LebiShop.Lebi_Language WXxg in WXxgs){%>
            <dd <%if (WXxg_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=WXxg.id%>,'<%=WXxg.Code%>','<%=WXxg.Path%>');"><%if (WXxg.ImageUrl!=""){%><img src="<%=Image(WXxg.ImageUrl) %>" /><%}%><%=WXxg.Name%></a></dd>
            <%WXxg_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int OzXc_index=1;
List<Lebi_Currency> OzXcs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency OzXc in OzXcs){%>
            <dd <%if (OzXc_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=OzXc.id%>,'<%=OzXc.Code%>',<%=OzXc.ExchangeRate%>,'<%=OzXc.Msige%>','<%=OzXc.DecimalLength%>');"><%=OzXc.Code%></a></dd>
            <%OzXc_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Cngl_index=1;
List<Lebi_Searchkey> Cngls = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Cngl in Cngls){%><%=Lang(Cngl.Name)%><%Cngl_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Cvvt_index=1;
List<Lebi_Searchkey> Cvvts = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Cvvt in Cvvts){%>
<%if (Cvvt.Type==1){ %><a href="<%=URL("P_Search",""+Lang(Cvvt.Name)+"") %>"><%}else{ %><a href="<%=Cvvt.URL%>" target="_blank"><%} %><span><%=Lang(Cvvt.Name)%></span></a>
<%Cvvt_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int Faft_index=1;
List<Lebi_Page> Fafts = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page Faft in Fafts){%>
<a class="menu" href="<%=URL("","",Faft.url)%>" target="<%=Faft.target%>"><span style="color:<%=Faft.NameColor%>"><%=Faft.Name%></span></a> 
<%Faft_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int nXmA_index=1;
List<Lebi_FriendLink> nXmAs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink nXmA in nXmAs){%>
            
                <% if (nXmA.Logo != "" && nXmA.IsPic == 1){ %><a href="<%=nXmA.Url%>" target="_blank"><img src="<%=Image(nXmA.Logo) %>" alt="<%=nXmA.Name%>" /></a><%}else{%><a href="<%=nXmA.Url%>" target="_blank"><%=nXmA.Name%></a><%} %>     
