...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> RsKDs=Languages();RecordCount=RsKDs.Count;int RsKD_index=1;
foreach (DB.LebiShop.Lebi_Language RsKD in RsKDs){%>
            <dd <%if (RsKD_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=RsKD.id%>,'<%=RsKD.Code%>','<%=RsKD.Path%>');"><%if (RsKD.ImageUrl!=""){%><img src="<%=Image(RsKD.ImageUrl) %>" /><%}%><%=RsKD.Name%></a></dd>
            <%RsKD_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int DkNh_index=1;
List<Lebi_Currency> DkNhs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency DkNh in DkNhs){%>
            <dd <%if (DkNh_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=DkNh.id%>,'<%=DkNh.Code%>',<%=DkNh.ExchangeRate%>,'<%=DkNh.Msige%>','<%=DkNh.DecimalLength%>');"><%=DkNh.Code%></a></dd>
            <%DkNh_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int tzLR_index=1;
List<Lebi_Searchkey> tzLRs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey tzLR in tzLRs){%><%=Lang(tzLR.Name)%><%tzLR_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int kApx_index=1;
List<Lebi_Searchkey> kApxs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey kApx in kApxs){%>
<%if (kApx.Type==1){ %><a href="<%=URL("P_Search",""+Lang(kApx.Name)+"") %>"><%}else{ %><a href="<%=kApx.URL%>" target="_blank"><%} %><span><%=Lang(kApx.Name)%></span></a>
<%kApx_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int xYiP_index=1;
List<Lebi_Page> xYiPs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page xYiP in xYiPs){%>
<a class="menu" href="<%=URL("","",xYiP.url)%>" target="<%=xYiP.target%>"><span style="color:<%=xYiP.NameColor%>"><%=xYiP.Name%></span></a> 
<%xYiP_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int FfUO_index=1;
List<Lebi_FriendLink> FfUOs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink FfUO in FfUOs){%>
            
                <% if (FfUO.Logo != "" && FfUO.IsPic == 1){ %><a href="<%=FfUO.Url%>" target="_blank"><img src="<%=Image(FfUO.Logo) %>" alt="<%=FfUO.Name%>" /></a><%}else{%><a href="<%=FfUO.Url%>" target="_blank"><%=FfUO.Name%></a><%} %>     
