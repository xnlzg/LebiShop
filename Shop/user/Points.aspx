...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> AMMVs=Languages();RecordCount=AMMVs.Count;int AMMV_index=1;
foreach (DB.LebiShop.Lebi_Language AMMV in AMMVs){%>
            <dd <%if (AMMV_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=AMMV.id%>,'<%=AMMV.Code%>','<%=AMMV.Path%>');"><%if (AMMV.ImageUrl!=""){%><img src="<%=Image(AMMV.ImageUrl) %>" /><%}%><%=AMMV.Name%></a></dd>
            <%AMMV_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int volu_index=1;
List<Lebi_Currency> volus = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency volu in volus){%>
            <dd <%if (volu_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=volu.id%>,'<%=volu.Code%>',<%=volu.ExchangeRate%>,'<%=volu.Msige%>','<%=volu.DecimalLength%>');"><%=volu.Code%></a></dd>
            <%volu_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int rIUX_index=1;
List<Lebi_Searchkey> rIUXs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey rIUX in rIUXs){%><%=Lang(rIUX.Name)%><%rIUX_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int GFNQ_index=1;
List<Lebi_Searchkey> GFNQs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey GFNQ in GFNQs){%>
<%if (GFNQ.Type==1){ %><a href="<%=URL("P_Search",""+Lang(GFNQ.Name)+"") %>"><%}else{ %><a href="<%=GFNQ.URL%>" target="_blank"><%} %><span><%=Lang(GFNQ.Name)%></span></a>
<%GFNQ_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int cGaD_index=1;
List<Lebi_Page> cGaDs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page cGaD in cGaDs){%>
<a class="menu" href="<%=URL("","",cGaD.url)%>" target="<%=cGaD.target%>"><span style="color:<%=cGaD.NameColor%>"><%=cGaD.Name%></span></a> 
<%cGaD_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int qPct_index=1;
List<Lebi_FriendLink> qPcts = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink qPct in qPcts){%>
            
                <% if (qPct.Logo != "" && qPct.IsPic == 1){ %><a href="<%=qPct.Url%>" target="_blank"><img src="<%=Image(qPct.Logo) %>" alt="<%=qPct.Name%>" /></a><%}else{%><a href="<%=qPct.Url%>" target="_blank"><%=qPct.Name%></a><%} %>     
