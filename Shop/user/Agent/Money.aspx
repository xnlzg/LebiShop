...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> fcLes=Languages();RecordCount=fcLes.Count;int fcLe_index=1;
foreach (DB.LebiShop.Lebi_Language fcLe in fcLes){%>
            <dd <%if (fcLe_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=fcLe.id%>,'<%=fcLe.Code%>','<%=fcLe.Path%>');"><%if (fcLe.ImageUrl!=""){%><img src="<%=Image(fcLe.ImageUrl) %>" /><%}%><%=fcLe.Name%></a></dd>
            <%fcLe_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int AQQd_index=1;
List<Lebi_Currency> AQQds = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency AQQd in AQQds){%>
            <dd <%if (AQQd_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=AQQd.id%>,'<%=AQQd.Code%>',<%=AQQd.ExchangeRate%>,'<%=AQQd.Msige%>','<%=AQQd.DecimalLength%>');"><%=AQQd.Code%></a></dd>
            <%AQQd_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int AvzA_index=1;
List<Lebi_Searchkey> AvzAs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey AvzA in AvzAs){%><%=Lang(AvzA.Name)%><%AvzA_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int QXlf_index=1;
List<Lebi_Searchkey> QXlfs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey QXlf in QXlfs){%>
<%if (QXlf.Type==1){ %><a href="<%=URL("P_Search",""+Lang(QXlf.Name)+"") %>"><%}else{ %><a href="<%=QXlf.URL%>" target="_blank"><%} %><span><%=Lang(QXlf.Name)%></span></a>
<%QXlf_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int MWjC_index=1;
List<Lebi_Page> MWjCs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page MWjC in MWjCs){%>
<a class="menu" href="<%=URL("","",MWjC.url)%>" target="<%=MWjC.target%>"><span style="color:<%=MWjC.NameColor%>"><%=MWjC.Name%></span></a> 
<%MWjC_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int fsQB_index=1;
List<Lebi_FriendLink> fsQBs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink fsQB in fsQBs){%>
            
                <% if (fsQB.Logo != "" && fsQB.IsPic == 1){ %><a href="<%=fsQB.Url%>" target="_blank"><img src="<%=Image(fsQB.Logo) %>" alt="<%=fsQB.Name%>" /></a><%}else{%><a href="<%=fsQB.Url%>" target="_blank"><%=fsQB.Name%></a><%} %>     
