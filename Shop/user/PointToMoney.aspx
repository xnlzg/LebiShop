...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> TTXCs=Languages();RecordCount=TTXCs.Count;int TTXC_index=1;
foreach (DB.LebiShop.Lebi_Language TTXC in TTXCs){%>
            <dd <%if (TTXC_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=TTXC.id%>,'<%=TTXC.Code%>','<%=TTXC.Path%>');"><%if (TTXC.ImageUrl!=""){%><img src="<%=Image(TTXC.ImageUrl) %>" /><%}%><%=TTXC.Name%></a></dd>
            <%TTXC_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int lkzV_index=1;
List<Lebi_Currency> lkzVs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency lkzV in lkzVs){%>
            <dd <%if (lkzV_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=lkzV.id%>,'<%=lkzV.Code%>',<%=lkzV.ExchangeRate%>,'<%=lkzV.Msige%>','<%=lkzV.DecimalLength%>');"><%=lkzV.Code%></a></dd>
            <%lkzV_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int nrRO_index=1;
List<Lebi_Searchkey> nrROs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey nrRO in nrROs){%><%=Lang(nrRO.Name)%><%nrRO_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int dpHY_index=1;
List<Lebi_Searchkey> dpHYs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey dpHY in dpHYs){%>
<%if (dpHY.Type==1){ %><a href="<%=URL("P_Search",""+Lang(dpHY.Name)+"") %>"><%}else{ %><a href="<%=dpHY.URL%>" target="_blank"><%} %><span><%=Lang(dpHY.Name)%></span></a>
<%dpHY_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int bBPB_index=1;
List<Lebi_Page> bBPBs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page bBPB in bBPBs){%>
<a class="menu" href="<%=URL("","",bBPB.url)%>" target="<%=bBPB.target%>"><span style="color:<%=bBPB.NameColor%>"><%=bBPB.Name%></span></a> 
<%bBPB_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int pnAn_index=1;
List<Lebi_FriendLink> pnAns = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink pnAn in pnAns){%>
            
                <% if (pnAn.Logo != "" && pnAn.IsPic == 1){ %><a href="<%=pnAn.Url%>" target="_blank"><img src="<%=Image(pnAn.Logo) %>" alt="<%=pnAn.Name%>" /></a><%}else{%><a href="<%=pnAn.Url%>" target="_blank"><%=pnAn.Name%></a><%} %>     
