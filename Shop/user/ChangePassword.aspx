...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>网站语言：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> IawXs=Languages();RecordCount=IawXs.Count;int IawX_index=1;
foreach (DB.LebiShop.Lebi_Language IawX in IawXs){%>
            <dd <%if (IawX_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=IawX.id%>,'<%=IawX.Code%>','<%=IawX.Path%>');"><%if (IawX.ImageUrl!=""){%><img src="<%=Image(IawX.ImageUrl) %>" /><%}%><%=IawX.Name%></a></dd>
            <%IawX_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>币种：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int nLfO_index=1;
List<Lebi_Currency> nLfOs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency nLfO in nLfOs){%>
            <dd <%if (nLfO_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=nLfO.id%>,'<%=nLfO.Code%>',<%=nLfO.ExchangeRate%>,'<%=nLfO.Msige%>','<%=nLfO.DecimalLength%>');"><%=nLfO.Code%></a></dd>
            <%nLfO_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int eJzx_index=1;
List<Lebi_Searchkey> eJzxs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey eJzx in eJzxs){%><%=Lang(eJzx.Name)%><%eJzx_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int lMVI_index=1;
List<Lebi_Searchkey> lMVIs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey lMVI in lMVIs){%>
<%if (lMVI.Type==1){ %><a href="<%=URL("P_Search",""+Lang(lMVI.Name)+"") %>"><%}else{ %><a href="<%=lMVI.URL%>" target="_blank"><%} %><span><%=Lang(lMVI.Name)%></span></a>
<%lMVI_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int kqgo_index=1;
List<Lebi_Page> kqgos = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page kqgo in kqgos){%>
<a class="menu" href="<%=URL("","",kqgo.url)%>" target="<%=kqgo.target%>"><span style="color:<%=kqgo.NameColor%>"><%=kqgo.Name%></span></a> 
<%kqgo_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int xvhX_index=1;
List<Lebi_FriendLink> xvhXs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink xvhX in xvhXs){%>
            
                <% if (xvhX.Logo != "" && xvhX.IsPic == 1){ %><a href="<%=xvhX.Url%>" target="_blank"><img src="<%=Image(xvhX.Logo) %>" alt="<%=xvhX.Name%>" /></a><%}else{%><a href="<%=xvhX.Url%>" target="_blank"><%=xvhX.Name%></a><%} %>     
