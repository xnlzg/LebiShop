...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> UfOrs=Languages();RecordCount=UfOrs.Count;int UfOr_index=1;
foreach (DB.LebiShop.Lebi_Language UfOr in UfOrs){%>
            <dd <%if (UfOr_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=UfOr.id%>,'<%=UfOr.Code%>','<%=UfOr.Path%>');"><%if (UfOr.ImageUrl!=""){%><img src="<%=Image(UfOr.ImageUrl) %>" /><%}%><%=UfOr.Name%></a></dd>
            <%UfOr_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int BRpQ_index=1;
List<Lebi_Currency> BRpQs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency BRpQ in BRpQs){%>
            <dd <%if (BRpQ_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=BRpQ.id%>,'<%=BRpQ.Code%>',<%=BRpQ.ExchangeRate%>,'<%=BRpQ.Msige%>','<%=BRpQ.DecimalLength%>');"><%=BRpQ.Code%></a></dd>
            <%BRpQ_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int YhFP_index=1;
List<Lebi_Searchkey> YhFPs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey YhFP in YhFPs){%><%=Lang(YhFP.Name)%><%YhFP_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int LMxN_index=1;
List<Lebi_Searchkey> LMxNs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey LMxN in LMxNs){%>
<%if (LMxN.Type==1){ %><a href="<%=URL("P_Search",""+Lang(LMxN.Name)+"") %>"><%}else{ %><a href="<%=LMxN.URL%>" target="_blank"><%} %><span><%=Lang(LMxN.Name)%></span></a>
<%LMxN_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int NuER_index=1;
List<Lebi_Page> NuERs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page NuER in NuERs){%>
<a class="menu" href="<%=URL("","",NuER.url)%>" target="<%=NuER.target%>"><span style="color:<%=NuER.NameColor%>"><%=NuER.Name%></span></a> 
<%NuER_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int BmAd_index=1;
List<Lebi_FriendLink> BmAds = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink BmAd in BmAds){%>
            
                <% if (BmAd.Logo != "" && BmAd.IsPic == 1){ %><a href="<%=BmAd.Url%>" target="_blank"><img src="<%=Image(BmAd.Logo) %>" alt="<%=BmAd.Name%>" /></a><%}else{%><a href="<%=BmAd.Url%>" target="_blank"><%=BmAd.Name%></a><%} %>     
