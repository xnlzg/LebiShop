...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> YUdps=Languages();RecordCount=YUdps.Count;int YUdp_index=1;
foreach (DB.LebiShop.Lebi_Language YUdp in YUdps){%>
            <dd <%if (YUdp_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=YUdp.id%>,'<%=YUdp.Code%>','<%=YUdp.Path%>');"><%if (YUdp.ImageUrl!=""){%><img src="<%=Image(YUdp.ImageUrl) %>" /><%}%><%=YUdp.Name%></a></dd>
            <%YUdp_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int weJS_index=1;
List<Lebi_Currency> weJSs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency weJS in weJSs){%>
            <dd <%if (weJS_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=weJS.id%>,'<%=weJS.Code%>',<%=weJS.ExchangeRate%>,'<%=weJS.Msige%>','<%=weJS.DecimalLength%>');"><%=weJS.Code%></a></dd>
            <%weJS_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int uzMl_index=1;
List<Lebi_Searchkey> uzMls = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey uzMl in uzMls){%><%=Lang(uzMl.Name)%><%uzMl_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int CLWB_index=1;
List<Lebi_Searchkey> CLWBs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey CLWB in CLWBs){%>
<%if (CLWB.Type==1){ %><a href="<%=URL("P_Search",""+Lang(CLWB.Name)+"") %>"><%}else{ %><a href="<%=CLWB.URL%>" target="_blank"><%} %><span><%=Lang(CLWB.Name)%></span></a>
<%CLWB_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int vPaI_index=1;
List<Lebi_Page> vPaIs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page vPaI in vPaIs){%>
<a class="menu" href="<%=URL("","",vPaI.url)%>" target="<%=vPaI.target%>"><span style="color:<%=vPaI.NameColor%>"><%=vPaI.Name%></span></a> 
<%vPaI_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int IBQp_index=1;
List<Lebi_FriendLink> IBQps = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink IBQp in IBQps){%>
            
                <% if (IBQp.Logo != "" && IBQp.IsPic == 1){ %><a href="<%=IBQp.Url%>" target="_blank"><img src="<%=Image(IBQp.Logo) %>" alt="<%=IBQp.Name%>" /></a><%}else{%><a href="<%=IBQp.Url%>" target="_blank"><%=IBQp.Name%></a><%} %>     
