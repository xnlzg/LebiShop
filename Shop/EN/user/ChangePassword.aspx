...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> JFuJs=Languages();RecordCount=JFuJs.Count;int JFuJ_index=1;
foreach (DB.LebiShop.Lebi_Language JFuJ in JFuJs){%>
            <dd <%if (JFuJ_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=JFuJ.id%>,'<%=JFuJ.Code%>','<%=JFuJ.Path%>');"><%if (JFuJ.ImageUrl!=""){%><img src="<%=Image(JFuJ.ImageUrl) %>" /><%}%><%=JFuJ.Name%></a></dd>
            <%JFuJ_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int NrKp_index=1;
List<Lebi_Currency> NrKps = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency NrKp in NrKps){%>
            <dd <%if (NrKp_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=NrKp.id%>,'<%=NrKp.Code%>',<%=NrKp.ExchangeRate%>,'<%=NrKp.Msige%>','<%=NrKp.DecimalLength%>');"><%=NrKp.Code%></a></dd>
            <%NrKp_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int noGr_index=1;
List<Lebi_Searchkey> noGrs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey noGr in noGrs){%><%=Lang(noGr.Name)%><%noGr_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int DAFI_index=1;
List<Lebi_Searchkey> DAFIs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey DAFI in DAFIs){%>
<%if (DAFI.Type==1){ %><a href="<%=URL("P_Search",""+Lang(DAFI.Name)+"") %>"><%}else{ %><a href="<%=DAFI.URL%>" target="_blank"><%} %><span><%=Lang(DAFI.Name)%></span></a>
<%DAFI_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int OJVv_index=1;
List<Lebi_Page> OJVvs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page OJVv in OJVvs){%>
<a class="menu" href="<%=URL("","",OJVv.url)%>" target="<%=OJVv.target%>"><span style="color:<%=OJVv.NameColor%>"><%=OJVv.Name%></span></a> 
<%OJVv_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int fDHT_index=1;
List<Lebi_FriendLink> fDHTs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink fDHT in fDHTs){%>
            
                <% if (fDHT.Logo != "" && fDHT.IsPic == 1){ %><a href="<%=fDHT.Url%>" target="_blank"><img src="<%=Image(fDHT.Logo) %>" alt="<%=fDHT.Name%>" /></a><%}else{%><a href="<%=fDHT.Url%>" target="_blank"><%=fDHT.Name%></a><%} %>     
