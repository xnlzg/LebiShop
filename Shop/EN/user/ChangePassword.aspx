...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> vafgs=Languages();RecordCount=vafgs.Count;int vafg_index=1;
foreach (DB.LebiShop.Lebi_Language vafg in vafgs){%>
            <dd <%if (vafg_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=vafg.id%>,'<%=vafg.Code%>','<%=vafg.Path%>');"><%if (vafg.ImageUrl!=""){%><img src="<%=Image(vafg.ImageUrl) %>" /><%}%><%=vafg.Name%></a></dd>
            <%vafg_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int fpkb_index=1;
List<Lebi_Currency> fpkbs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency fpkb in fpkbs){%>
            <dd <%if (fpkb_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=fpkb.id%>,'<%=fpkb.Code%>',<%=fpkb.ExchangeRate%>,'<%=fpkb.Msige%>','<%=fpkb.DecimalLength%>');"><%=fpkb.Code%></a></dd>
            <%fpkb_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int OswA_index=1;
List<Lebi_Searchkey> OswAs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey OswA in OswAs){%><%=Lang(OswA.Name)%><%OswA_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int xhtS_index=1;
List<Lebi_Searchkey> xhtSs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey xhtS in xhtSs){%>
<%if (xhtS.Type==1){ %><a href="<%=URL("P_Search",""+Lang(xhtS.Name)+"") %>"><%}else{ %><a href="<%=xhtS.URL%>" target="_blank"><%} %><span><%=Lang(xhtS.Name)%></span></a>
<%xhtS_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int DCDI_index=1;
List<Lebi_Page> DCDIs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page DCDI in DCDIs){%>
<a class="menu" href="<%=URL("","",DCDI.url)%>" target="<%=DCDI.target%>"><span style="color:<%=DCDI.NameColor%>"><%=DCDI.Name%></span></a> 
<%DCDI_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int PyAG_index=1;
List<Lebi_FriendLink> PyAGs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink PyAG in PyAGs){%>
            
                <% if (PyAG.Logo != "" && PyAG.IsPic == 1){ %><a href="<%=PyAG.Url%>" target="_blank"><img src="<%=Image(PyAG.Logo) %>" alt="<%=PyAG.Name%>" /></a><%}else{%><a href="<%=PyAG.Url%>" target="_blank"><%=PyAG.Name%></a><%} %>     
