...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> qYBvs=Languages();RecordCount=qYBvs.Count;int qYBv_index=1;
foreach (DB.LebiShop.Lebi_Language qYBv in qYBvs){%>
            <dd <%if (qYBv_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=qYBv.id%>,'<%=qYBv.Code%>','<%=qYBv.Path%>');"><%if (qYBv.ImageUrl!=""){%><img src="<%=Image(qYBv.ImageUrl) %>" /><%}%><%=qYBv.Name%></a></dd>
            <%qYBv_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int PWhz_index=1;
List<Lebi_Currency> PWhzs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency PWhz in PWhzs){%>
            <dd <%if (PWhz_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=PWhz.id%>,'<%=PWhz.Code%>',<%=PWhz.ExchangeRate%>,'<%=PWhz.Msige%>','<%=PWhz.DecimalLength%>');"><%=PWhz.Code%></a></dd>
            <%PWhz_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int GYmc_index=1;
List<Lebi_Searchkey> GYmcs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey GYmc in GYmcs){%><%=Lang(GYmc.Name)%><%GYmc_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int NjIJ_index=1;
List<Lebi_Searchkey> NjIJs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey NjIJ in NjIJs){%>
<%if (NjIJ.Type==1){ %><a href="<%=URL("P_Search",""+Lang(NjIJ.Name)+"") %>"><%}else{ %><a href="<%=NjIJ.URL%>" target="_blank"><%} %><span><%=Lang(NjIJ.Name)%></span></a>
<%NjIJ_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int fwnK_index=1;
List<Lebi_Page> fwnKs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page fwnK in fwnKs){%>
<a class="menu" href="<%=URL("","",fwnK.url)%>" target="<%=fwnK.target%>"><span style="color:<%=fwnK.NameColor%>"><%=fwnK.Name%></span></a> 
<%fwnK_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int yPBk_index=1;
List<Lebi_FriendLink> yPBks = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink yPBk in yPBks){%>
            
                <% if (yPBk.Logo != "" && yPBk.IsPic == 1){ %><a href="<%=yPBk.Url%>" target="_blank"><img src="<%=Image(yPBk.Logo) %>" alt="<%=yPBk.Name%>" /></a><%}else{%><a href="<%=yPBk.Url%>" target="_blank"><%=yPBk.Name%></a><%} %>     
