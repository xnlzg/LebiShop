...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> ScWds=Languages();RecordCount=ScWds.Count;int ScWd_index=1;
foreach (DB.LebiShop.Lebi_Language ScWd in ScWds){%>
            <dd <%if (ScWd_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=ScWd.id%>,'<%=ScWd.Code%>','<%=ScWd.Path%>');"><%if (ScWd.ImageUrl!=""){%><img src="<%=Image(ScWd.ImageUrl) %>" /><%}%><%=ScWd.Name%></a></dd>
            <%ScWd_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int cznn_index=1;
List<Lebi_Currency> cznns = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency cznn in cznns){%>
            <dd <%if (cznn_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=cznn.id%>,'<%=cznn.Code%>',<%=cznn.ExchangeRate%>,'<%=cznn.Msige%>','<%=cznn.DecimalLength%>');"><%=cznn.Code%></a></dd>
            <%cznn_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int CmST_index=1;
List<Lebi_Searchkey> CmSTs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey CmST in CmSTs){%><%=Lang(CmST.Name)%><%CmST_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int AvzM_index=1;
List<Lebi_Searchkey> AvzMs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey AvzM in AvzMs){%>
<%if (AvzM.Type==1){ %><a href="<%=URL("P_Search",""+Lang(AvzM.Name)+"") %>"><%}else{ %><a href="<%=AvzM.URL%>" target="_blank"><%} %><span><%=Lang(AvzM.Name)%></span></a>
<%AvzM_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int OoJL_index=1;
List<Lebi_Page> OoJLs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page OoJL in OoJLs){%>
<a class="menu" href="<%=URL("","",OoJL.url)%>" target="<%=OoJL.target%>"><span style="color:<%=OoJL.NameColor%>"><%=OoJL.Name%></span></a> 
<%OoJL_index++;}%>
...
<div id="orderstate">
    <div class="mt">
        ...
        <div class="right">
            <div class="toolbar">
                <%if(Shop.Bussiness.Order.CancelOrder(order)){ %><a href="javascript:void(0)" onclick="OrderCancel(<%=order.id %>);" class="btn btn-11"><s></s>Cancel</a><%} %>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int zjEq_index=1;
List<Lebi_FriendLink> zjEqs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink zjEq in zjEqs){%>
            
                <% if (zjEq.Logo != "" && zjEq.IsPic == 1){ %><a href="<%=zjEq.Url%>" target="_blank"><img src="<%=Image(zjEq.Logo) %>" alt="<%=zjEq.Name%>" /></a><%}else{%><a href="<%=zjEq.Url%>" target="_blank"><%=zjEq.Name%></a><%} %>     
