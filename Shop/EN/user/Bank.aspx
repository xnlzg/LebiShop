...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> SGGCs=Languages();RecordCount=SGGCs.Count;int SGGC_index=1;
foreach (DB.LebiShop.Lebi_Language SGGC in SGGCs){%>
            <dd <%if (SGGC_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=SGGC.id%>,'<%=SGGC.Code%>','<%=SGGC.Path%>');"><%if (SGGC.ImageUrl!=""){%><img src="<%=Image(SGGC.ImageUrl) %>" /><%}%><%=SGGC.Name%></a></dd>
            <%SGGC_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int mcvB_index=1;
List<Lebi_Currency> mcvBs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency mcvB in mcvBs){%>
            <dd <%if (mcvB_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=mcvB.id%>,'<%=mcvB.Code%>',<%=mcvB.ExchangeRate%>,'<%=mcvB.Msige%>','<%=mcvB.DecimalLength%>');"><%=mcvB.Code%></a></dd>
            <%mcvB_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int JVkf_index=1;
List<Lebi_Searchkey> JVkfs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey JVkf in JVkfs){%><%=Lang(JVkf.Name)%><%JVkf_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int ebhc_index=1;
List<Lebi_Searchkey> ebhcs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey ebhc in ebhcs){%>
<%if (ebhc.Type==1){ %><a href="<%=URL("P_Search",""+Lang(ebhc.Name)+"") %>"><%}else{ %><a href="<%=ebhc.URL%>" target="_blank"><%} %><span><%=Lang(ebhc.Name)%></span></a>
<%ebhc_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int rIGy_index=1;
List<Lebi_Page> rIGys = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page rIGy in rIGys){%>
<a class="menu" href="<%=URL("","",rIGy.url)%>" target="<%=rIGy.target%>"><span style="color:<%=rIGy.NameColor%>"><%=rIGy.Name%></span></a> 
<%rIGy_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int RYgF_index=1;
List<Lebi_FriendLink> RYgFs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink RYgF in RYgFs){%>
            
                <% if (RYgF.Logo != "" && RYgF.IsPic == 1){ %><a href="<%=RYgF.Url%>" target="_blank"><img src="<%=Image(RYgF.Logo) %>" alt="<%=RYgF.Name%>" /></a><%}else{%><a href="<%=RYgF.Url%>" target="_blank"><%=RYgF.Name%></a><%} %>     
