...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> KghMs=Languages();RecordCount=KghMs.Count;int KghM_index=1;
foreach (DB.LebiShop.Lebi_Language KghM in KghMs){%>
            <dd <%if (KghM_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=KghM.id%>,'<%=KghM.Code%>','<%=KghM.Path%>');"><%if (KghM.ImageUrl!=""){%><img src="<%=Image(KghM.ImageUrl) %>" /><%}%><%=KghM.Name%></a></dd>
            <%KghM_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int xmkB_index=1;
List<Lebi_Currency> xmkBs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency xmkB in xmkBs){%>
            <dd <%if (xmkB_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=xmkB.id%>,'<%=xmkB.Code%>',<%=xmkB.ExchangeRate%>,'<%=xmkB.Msige%>','<%=xmkB.DecimalLength%>');"><%=xmkB.Code%></a></dd>
            <%xmkB_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int DVzt_index=1;
List<Lebi_Searchkey> DVzts = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey DVzt in DVzts){%><%=Lang(DVzt.Name)%><%DVzt_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int htGs_index=1;
List<Lebi_Searchkey> htGss = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey htGs in htGss){%>
<%if (htGs.Type==1){ %><a href="<%=URL("P_Search",""+Lang(htGs.Name)+"") %>"><%}else{ %><a href="<%=htGs.URL%>" target="_blank"><%} %><span><%=Lang(htGs.Name)%></span></a>
<%htGs_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int PYOL_index=1;
List<Lebi_Page> PYOLs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page PYOL in PYOLs){%>
<a class="menu" href="<%=URL("","",PYOL.url)%>" target="<%=PYOL.target%>"><span style="color:<%=PYOL.NameColor%>"><%=PYOL.Name%></span></a> 
<%PYOL_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int yttG_index=1;
List<Lebi_FriendLink> yttGs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink yttG in yttGs){%>
            
                <% if (yttG.Logo != "" && yttG.IsPic == 1){ %><a href="<%=yttG.Url%>" target="_blank"><img src="<%=Image(yttG.Logo) %>" alt="<%=yttG.Name%>" /></a><%}else{%><a href="<%=yttG.Url%>" target="_blank"><%=yttG.Name%></a><%} %>     
