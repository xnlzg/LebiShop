...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> FiuSs=Languages();RecordCount=FiuSs.Count;int FiuS_index=1;
foreach (DB.LebiShop.Lebi_Language FiuS in FiuSs){%>
            <dd <%if (FiuS_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=FiuS.id%>,'<%=FiuS.Code%>','<%=FiuS.Path%>');"><%if (FiuS.ImageUrl!=""){%><img src="<%=Image(FiuS.ImageUrl) %>" /><%}%><%=FiuS.Name%></a></dd>
            <%FiuS_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int Ullp_index=1;
List<Lebi_Currency> Ullps = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency Ullp in Ullps){%>
            <dd <%if (Ullp_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=Ullp.id%>,'<%=Ullp.Code%>',<%=Ullp.ExchangeRate%>,'<%=Ullp.Msige%>','<%=Ullp.DecimalLength%>');"><%=Ullp.Code%></a></dd>
            <%Ullp_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int vxEb_index=1;
List<Lebi_Searchkey> vxEbs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey vxEb in vxEbs){%><%=Lang(vxEb.Name)%><%vxEb_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int wbpL_index=1;
List<Lebi_Searchkey> wbpLs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey wbpL in wbpLs){%>
<%if (wbpL.Type==1){ %><a href="<%=URL("P_Search",""+Lang(wbpL.Name)+"") %>"><%}else{ %><a href="<%=wbpL.URL%>" target="_blank"><%} %><span><%=Lang(wbpL.Name)%></span></a>
<%wbpL_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int lvID_index=1;
List<Lebi_Page> lvIDs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page lvID in lvIDs){%>
<a class="menu" href="<%=URL("","",lvID.url)%>" target="<%=lvID.target%>"><span style="color:<%=lvID.NameColor%>"><%=lvID.Name%></span></a> 
<%lvID_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int gUlV_index=1;
List<Lebi_FriendLink> gUlVs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink gUlV in gUlVs){%>
            
                <% if (gUlV.Logo != "" && gUlV.IsPic == 1){ %><a href="<%=gUlV.Url%>" target="_blank"><img src="<%=Image(gUlV.Logo) %>" alt="<%=gUlV.Name%>" /></a><%}else{%><a href="<%=gUlV.Url%>" target="_blank"><%=gUlV.Name%></a><%} %>     
