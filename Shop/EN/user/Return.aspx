...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> JRnXs=Languages();RecordCount=JRnXs.Count;int JRnX_index=1;
foreach (DB.LebiShop.Lebi_Language JRnX in JRnXs){%>
            <dd <%if (JRnX_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=JRnX.id%>,'<%=JRnX.Code%>','<%=JRnX.Path%>');"><%if (JRnX.ImageUrl!=""){%><img src="<%=Image(JRnX.ImageUrl) %>" /><%}%><%=JRnX.Name%></a></dd>
            <%JRnX_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int nzJR_index=1;
List<Lebi_Currency> nzJRs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency nzJR in nzJRs){%>
            <dd <%if (nzJR_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=nzJR.id%>,'<%=nzJR.Code%>',<%=nzJR.ExchangeRate%>,'<%=nzJR.Msige%>','<%=nzJR.DecimalLength%>');"><%=nzJR.Code%></a></dd>
            <%nzJR_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int YsPX_index=1;
List<Lebi_Searchkey> YsPXs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey YsPX in YsPXs){%><%=Lang(YsPX.Name)%><%YsPX_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int OHzp_index=1;
List<Lebi_Searchkey> OHzps = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey OHzp in OHzps){%>
<%if (OHzp.Type==1){ %><a href="<%=URL("P_Search",""+Lang(OHzp.Name)+"") %>"><%}else{ %><a href="<%=OHzp.URL%>" target="_blank"><%} %><span><%=Lang(OHzp.Name)%></span></a>
<%OHzp_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int iPed_index=1;
List<Lebi_Page> iPeds = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page iPed in iPeds){%>
<a class="menu" href="<%=URL("","",iPed.url)%>" target="<%=iPed.target%>"><span style="color:<%=iPed.NameColor%>"><%=iPed.Name%></span></a> 
<%iPed_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int qQqJ_index=1;
List<Lebi_FriendLink> qQqJs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink qQqJ in qQqJs){%>
            
                <% if (qQqJ.Logo != "" && qQqJ.IsPic == 1){ %><a href="<%=qQqJ.Url%>" target="_blank"><img src="<%=Image(qQqJ.Logo) %>" alt="<%=qQqJ.Name%>" /></a><%}else{%><a href="<%=qQqJ.Url%>" target="_blank"><%=qQqJ.Name%></a><%} %>     
