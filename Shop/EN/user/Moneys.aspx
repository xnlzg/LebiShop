...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> DcRds=Languages();RecordCount=DcRds.Count;int DcRd_index=1;
foreach (DB.LebiShop.Lebi_Language DcRd in DcRds){%>
            <dd <%if (DcRd_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=DcRd.id%>,'<%=DcRd.Code%>','<%=DcRd.Path%>');"><%if (DcRd.ImageUrl!=""){%><img src="<%=Image(DcRd.ImageUrl) %>" /><%}%><%=DcRd.Name%></a></dd>
            <%DcRd_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int WTPI_index=1;
List<Lebi_Currency> WTPIs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency WTPI in WTPIs){%>
            <dd <%if (WTPI_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=WTPI.id%>,'<%=WTPI.Code%>',<%=WTPI.ExchangeRate%>,'<%=WTPI.Msige%>','<%=WTPI.DecimalLength%>');"><%=WTPI.Code%></a></dd>
            <%WTPI_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int hbpE_index=1;
List<Lebi_Searchkey> hbpEs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey hbpE in hbpEs){%><%=Lang(hbpE.Name)%><%hbpE_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int HyoS_index=1;
List<Lebi_Searchkey> HyoSs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey HyoS in HyoSs){%>
<%if (HyoS.Type==1){ %><a href="<%=URL("P_Search",""+Lang(HyoS.Name)+"") %>"><%}else{ %><a href="<%=HyoS.URL%>" target="_blank"><%} %><span><%=Lang(HyoS.Name)%></span></a>
<%HyoS_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int JNPA_index=1;
List<Lebi_Page> JNPAs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page JNPA in JNPAs){%>
<a class="menu" href="<%=URL("","",JNPA.url)%>" target="<%=JNPA.target%>"><span style="color:<%=JNPA.NameColor%>"><%=JNPA.Name%></span></a> 
<%JNPA_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int iNaU_index=1;
List<Lebi_FriendLink> iNaUs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink iNaU in iNaUs){%>
            
                <% if (iNaU.Logo != "" && iNaU.IsPic == 1){ %><a href="<%=iNaU.Url%>" target="_blank"><img src="<%=Image(iNaU.Logo) %>" alt="<%=iNaU.Name%>" /></a><%}else{%><a href="<%=iNaU.Url%>" target="_blank"><%=iNaU.Name%></a><%} %>     
