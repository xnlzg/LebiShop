...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> wMVCs=Languages();RecordCount=wMVCs.Count;int wMVC_index=1;
foreach (DB.LebiShop.Lebi_Language wMVC in wMVCs){%>
            <dd <%if (wMVC_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=wMVC.id%>,'<%=wMVC.Code%>','<%=wMVC.Path%>');"><%if (wMVC.ImageUrl!=""){%><img src="<%=Image(wMVC.ImageUrl) %>" /><%}%><%=wMVC.Name%></a></dd>
            <%wMVC_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int lEeM_index=1;
List<Lebi_Currency> lEeMs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency lEeM in lEeMs){%>
            <dd <%if (lEeM_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=lEeM.id%>,'<%=lEeM.Code%>',<%=lEeM.ExchangeRate%>,'<%=lEeM.Msige%>','<%=lEeM.DecimalLength%>');"><%=lEeM.Code%></a></dd>
            <%lEeM_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int vXFf_index=1;
List<Lebi_Searchkey> vXFfs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey vXFf in vXFfs){%><%=Lang(vXFf.Name)%><%vXFf_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int ChDB_index=1;
List<Lebi_Searchkey> ChDBs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey ChDB in ChDBs){%>
<%if (ChDB.Type==1){ %><a href="<%=URL("P_Search",""+Lang(ChDB.Name)+"") %>"><%}else{ %><a href="<%=ChDB.URL%>" target="_blank"><%} %><span><%=Lang(ChDB.Name)%></span></a>
<%ChDB_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int NNlO_index=1;
List<Lebi_Page> NNlOs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page NNlO in NNlOs){%>
<a class="menu" href="<%=URL("","",NNlO.url)%>" target="<%=NNlO.target%>"><span style="color:<%=NNlO.NameColor%>"><%=NNlO.Name%></span></a> 
<%NNlO_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int YWcf_index=1;
List<Lebi_FriendLink> YWcfs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink YWcf in YWcfs){%>
            
                <% if (YWcf.Logo != "" && YWcf.IsPic == 1){ %><a href="<%=YWcf.Url%>" target="_blank"><img src="<%=Image(YWcf.Logo) %>" alt="<%=YWcf.Name%>" /></a><%}else{%><a href="<%=YWcf.Url%>" target="_blank"><%=YWcf.Name%></a><%} %>     
