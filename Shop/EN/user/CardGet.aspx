...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> qpsys=Languages();RecordCount=qpsys.Count;int qpsy_index=1;
foreach (DB.LebiShop.Lebi_Language qpsy in qpsys){%>
            <dd <%if (qpsy_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=qpsy.id%>,'<%=qpsy.Code%>','<%=qpsy.Path%>');"><%if (qpsy.ImageUrl!=""){%><img src="<%=Image(qpsy.ImageUrl) %>" /><%}%><%=qpsy.Name%></a></dd>
            <%qpsy_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int nauH_index=1;
List<Lebi_Currency> nauHs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency nauH in nauHs){%>
            <dd <%if (nauH_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=nauH.id%>,'<%=nauH.Code%>',<%=nauH.ExchangeRate%>,'<%=nauH.Msige%>','<%=nauH.DecimalLength%>');"><%=nauH.Code%></a></dd>
            <%nauH_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int fPnr_index=1;
List<Lebi_Searchkey> fPnrs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey fPnr in fPnrs){%><%=Lang(fPnr.Name)%><%fPnr_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int ivoc_index=1;
List<Lebi_Searchkey> ivocs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey ivoc in ivocs){%>
<%if (ivoc.Type==1){ %><a href="<%=URL("P_Search",""+Lang(ivoc.Name)+"") %>"><%}else{ %><a href="<%=ivoc.URL%>" target="_blank"><%} %><span><%=Lang(ivoc.Name)%></span></a>
<%ivoc_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int JLLS_index=1;
List<Lebi_Page> JLLSs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page JLLS in JLLSs){%>
<a class="menu" href="<%=URL("","",JLLS.url)%>" target="<%=JLLS.target%>"><span style="color:<%=JLLS.NameColor%>"><%=JLLS.Name%></span></a> 
<%JLLS_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int QhGS_index=1;
List<Lebi_FriendLink> QhGSs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink QhGS in QhGSs){%>
            
                <% if (QhGS.Logo != "" && QhGS.IsPic == 1){ %><a href="<%=QhGS.Url%>" target="_blank"><img src="<%=Image(QhGS.Logo) %>" alt="<%=QhGS.Name%>" /></a><%}else{%><a href="<%=QhGS.Url%>" target="_blank"><%=QhGS.Name%></a><%} %>     
