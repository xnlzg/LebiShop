...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> Kwois=Languages();RecordCount=Kwois.Count;int Kwoi_index=1;
foreach (DB.LebiShop.Lebi_Language Kwoi in Kwois){%>
            <dd <%if (Kwoi_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=Kwoi.id%>,'<%=Kwoi.Code%>','<%=Kwoi.Path%>');"><%if (Kwoi.ImageUrl!=""){%><img src="<%=Image(Kwoi.ImageUrl) %>" /><%}%><%=Kwoi.Name%></a></dd>
            <%Kwoi_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int ELfy_index=1;
List<Lebi_Currency> ELfys = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency ELfy in ELfys){%>
            <dd <%if (ELfy_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=ELfy.id%>,'<%=ELfy.Code%>',<%=ELfy.ExchangeRate%>,'<%=ELfy.Msige%>','<%=ELfy.DecimalLength%>');"><%=ELfy.Code%></a></dd>
            <%ELfy_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int kydd_index=1;
List<Lebi_Searchkey> kydds = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey kydd in kydds){%><%=Lang(kydd.Name)%><%kydd_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int OJJY_index=1;
List<Lebi_Searchkey> OJJYs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey OJJY in OJJYs){%>
<%if (OJJY.Type==1){ %><a href="<%=URL("P_Search",""+Lang(OJJY.Name)+"") %>"><%}else{ %><a href="<%=OJJY.URL%>" target="_blank"><%} %><span><%=Lang(OJJY.Name)%></span></a>
<%OJJY_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int mmFv_index=1;
List<Lebi_Page> mmFvs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page mmFv in mmFvs){%>
<a class="menu" href="<%=URL("","",mmFv.url)%>" target="<%=mmFv.target%>"><span style="color:<%=mmFv.NameColor%>"><%=mmFv.Name%></span></a> 
<%mmFv_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int JNml_index=1;
List<Lebi_FriendLink> JNmls = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink JNml in JNmls){%>
            
                <% if (JNml.Logo != "" && JNml.IsPic == 1){ %><a href="<%=JNml.Url%>" target="_blank"><img src="<%=Image(JNml.Logo) %>" alt="<%=JNml.Name%>" /></a><%}else{%><a href="<%=JNml.Url%>" target="_blank"><%=JNml.Name%></a><%} %>     
