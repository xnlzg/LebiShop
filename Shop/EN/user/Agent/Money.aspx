...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> DMoXs=Languages();RecordCount=DMoXs.Count;int DMoX_index=1;
foreach (DB.LebiShop.Lebi_Language DMoX in DMoXs){%>
            <dd <%if (DMoX_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=DMoX.id%>,'<%=DMoX.Code%>','<%=DMoX.Path%>');"><%if (DMoX.ImageUrl!=""){%><img src="<%=Image(DMoX.ImageUrl) %>" /><%}%><%=DMoX.Name%></a></dd>
            <%DMoX_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int TJRJ_index=1;
List<Lebi_Currency> TJRJs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency TJRJ in TJRJs){%>
            <dd <%if (TJRJ_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=TJRJ.id%>,'<%=TJRJ.Code%>',<%=TJRJ.ExchangeRate%>,'<%=TJRJ.Msige%>','<%=TJRJ.DecimalLength%>');"><%=TJRJ.Code%></a></dd>
            <%TJRJ_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int EGPz_index=1;
List<Lebi_Searchkey> EGPzs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey EGPz in EGPzs){%><%=Lang(EGPz.Name)%><%EGPz_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Lzah_index=1;
List<Lebi_Searchkey> Lzahs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Lzah in Lzahs){%>
<%if (Lzah.Type==1){ %><a href="<%=URL("P_Search",""+Lang(Lzah.Name)+"") %>"><%}else{ %><a href="<%=Lzah.URL%>" target="_blank"><%} %><span><%=Lang(Lzah.Name)%></span></a>
<%Lzah_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int CKAE_index=1;
List<Lebi_Page> CKAEs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page CKAE in CKAEs){%>
<a class="menu" href="<%=URL("","",CKAE.url)%>" target="<%=CKAE.target%>"><span style="color:<%=CKAE.NameColor%>"><%=CKAE.Name%></span></a> 
<%CKAE_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int zjWR_index=1;
List<Lebi_FriendLink> zjWRs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink zjWR in zjWRs){%>
            
                <% if (zjWR.Logo != "" && zjWR.IsPic == 1){ %><a href="<%=zjWR.Url%>" target="_blank"><img src="<%=Image(zjWR.Logo) %>" alt="<%=zjWR.Name%>" /></a><%}else{%><a href="<%=zjWR.Url%>" target="_blank"><%=zjWR.Name%></a><%} %>     
