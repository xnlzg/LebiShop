...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> EvKks=Languages();RecordCount=EvKks.Count;int EvKk_index=1;
foreach (DB.LebiShop.Lebi_Language EvKk in EvKks){%>
            <dd <%if (EvKk_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=EvKk.id%>,'<%=EvKk.Code%>','<%=EvKk.Path%>');"><%if (EvKk.ImageUrl!=""){%><img src="<%=Image(EvKk.ImageUrl) %>" /><%}%><%=EvKk.Name%></a></dd>
            <%EvKk_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int Qvjx_index=1;
List<Lebi_Currency> Qvjxs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency Qvjx in Qvjxs){%>
            <dd <%if (Qvjx_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=Qvjx.id%>,'<%=Qvjx.Code%>',<%=Qvjx.ExchangeRate%>,'<%=Qvjx.Msige%>','<%=Qvjx.DecimalLength%>');"><%=Qvjx.Code%></a></dd>
            <%Qvjx_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int lWOx_index=1;
List<Lebi_Searchkey> lWOxs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey lWOx in lWOxs){%><%=Lang(lWOx.Name)%><%lWOx_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int AHNL_index=1;
List<Lebi_Searchkey> AHNLs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey AHNL in AHNLs){%>
<%if (AHNL.Type==1){ %><a href="<%=URL("P_Search",""+Lang(AHNL.Name)+"") %>"><%}else{ %><a href="<%=AHNL.URL%>" target="_blank"><%} %><span><%=Lang(AHNL.Name)%></span></a>
<%AHNL_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int ANWH_index=1;
List<Lebi_Page> ANWHs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page ANWH in ANWHs){%>
<a class="menu" href="<%=URL("","",ANWH.url)%>" target="<%=ANWH.target%>"><span style="color:<%=ANWH.NameColor%>"><%=ANWH.Name%></span></a> 
<%ANWH_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int wkYh_index=1;
List<Lebi_FriendLink> wkYhs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink wkYh in wkYhs){%>
            
                <% if (wkYh.Logo != "" && wkYh.IsPic == 1){ %><a href="<%=wkYh.Url%>" target="_blank"><img src="<%=Image(wkYh.Logo) %>" alt="<%=wkYh.Name%>" /></a><%}else{%><a href="<%=wkYh.Url%>" target="_blank"><%=wkYh.Name%></a><%} %>     
