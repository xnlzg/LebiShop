...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> bgcws=Languages();RecordCount=bgcws.Count;int bgcw_index=1;
foreach (DB.LebiShop.Lebi_Language bgcw in bgcws){%>
            <dd <%if (bgcw_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=bgcw.id%>,'<%=bgcw.Code%>','<%=bgcw.Path%>');"><%if (bgcw.ImageUrl!=""){%><img src="<%=Image(bgcw.ImageUrl) %>" /><%}%><%=bgcw.Name%></a></dd>
            <%bgcw_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int aTcz_index=1;
List<Lebi_Currency> aTczs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency aTcz in aTczs){%>
            <dd <%if (aTcz_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=aTcz.id%>,'<%=aTcz.Code%>',<%=aTcz.ExchangeRate%>,'<%=aTcz.Msige%>','<%=aTcz.DecimalLength%>');"><%=aTcz.Code%></a></dd>
            <%aTcz_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int hCDp_index=1;
List<Lebi_Searchkey> hCDps = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey hCDp in hCDps){%><%=Lang(hCDp.Name)%><%hCDp_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int WipA_index=1;
List<Lebi_Searchkey> WipAs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey WipA in WipAs){%>
<%if (WipA.Type==1){ %><a href="<%=URL("P_Search",""+Lang(WipA.Name)+"") %>"><%}else{ %><a href="<%=WipA.URL%>" target="_blank"><%} %><span><%=Lang(WipA.Name)%></span></a>
<%WipA_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int MMol_index=1;
List<Lebi_Page> MMols = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page MMol in MMols){%>
<a class="menu" href="<%=URL("","",MMol.url)%>" target="<%=MMol.target%>"><span style="color:<%=MMol.NameColor%>"><%=MMol.Name%></span></a> 
<%MMol_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int fvho_index=1;
List<Lebi_FriendLink> fvhos = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink fvho in fvhos){%>
            
                <% if (fvho.Logo != "" && fvho.IsPic == 1){ %><a href="<%=fvho.Url%>" target="_blank"><img src="<%=Image(fvho.Logo) %>" alt="<%=fvho.Name%>" /></a><%}else{%><a href="<%=fvho.Url%>" target="_blank"><%=fvho.Name%></a><%} %>     
