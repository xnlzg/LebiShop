...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> DYIAs=Languages();RecordCount=DYIAs.Count;int DYIA_index=1;
foreach (DB.LebiShop.Lebi_Language DYIA in DYIAs){%>
            <dd <%if (DYIA_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=DYIA.id%>,'<%=DYIA.Code%>','<%=DYIA.Path%>');"><%if (DYIA.ImageUrl!=""){%><img src="<%=Image(DYIA.ImageUrl) %>" /><%}%><%=DYIA.Name%></a></dd>
            <%DYIA_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int yoLC_index=1;
List<Lebi_Currency> yoLCs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency yoLC in yoLCs){%>
            <dd <%if (yoLC_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=yoLC.id%>,'<%=yoLC.Code%>',<%=yoLC.ExchangeRate%>,'<%=yoLC.Msige%>','<%=yoLC.DecimalLength%>');"><%=yoLC.Code%></a></dd>
            <%yoLC_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int DBcD_index=1;
List<Lebi_Searchkey> DBcDs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey DBcD in DBcDs){%><%=Lang(DBcD.Name)%><%DBcD_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int taJR_index=1;
List<Lebi_Searchkey> taJRs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey taJR in taJRs){%>
<%if (taJR.Type==1){ %><a href="<%=URL("P_Search",""+Lang(taJR.Name)+"") %>"><%}else{ %><a href="<%=taJR.URL%>" target="_blank"><%} %><span><%=Lang(taJR.Name)%></span></a>
<%taJR_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int EOSP_index=1;
List<Lebi_Page> EOSPs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page EOSP in EOSPs){%>
<a class="menu" href="<%=URL("","",EOSP.url)%>" target="<%=EOSP.target%>"><span style="color:<%=EOSP.NameColor%>"><%=EOSP.Name%></span></a> 
<%EOSP_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int ntIK_index=1;
List<Lebi_FriendLink> ntIKs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink ntIK in ntIKs){%>
            
                <% if (ntIK.Logo != "" && ntIK.IsPic == 1){ %><a href="<%=ntIK.Url%>" target="_blank"><img src="<%=Image(ntIK.Logo) %>" alt="<%=ntIK.Name%>" /></a><%}else{%><a href="<%=ntIK.Url%>" target="_blank"><%=ntIK.Name%></a><%} %>     
