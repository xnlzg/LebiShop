...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> TssMs=Languages();RecordCount=TssMs.Count;int TssM_index=1;
foreach (DB.LebiShop.Lebi_Language TssM in TssMs){%>
            <dd <%if (TssM_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=TssM.id%>,'<%=TssM.Code%>','<%=TssM.Path%>');"><%if (TssM.ImageUrl!=""){%><img src="<%=Image(TssM.ImageUrl) %>" /><%}%><%=TssM.Name%></a></dd>
            <%TssM_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int JCju_index=1;
List<Lebi_Currency> JCjus = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency JCju in JCjus){%>
            <dd <%if (JCju_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=JCju.id%>,'<%=JCju.Code%>',<%=JCju.ExchangeRate%>,'<%=JCju.Msige%>','<%=JCju.DecimalLength%>');"><%=JCju.Code%></a></dd>
            <%JCju_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int jnLJ_index=1;
List<Lebi_Searchkey> jnLJs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey jnLJ in jnLJs){%><%=Lang(jnLJ.Name)%><%jnLJ_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Hdkm_index=1;
List<Lebi_Searchkey> Hdkms = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Hdkm in Hdkms){%>
<%if (Hdkm.Type==1){ %><a href="<%=URL("P_Search",""+Lang(Hdkm.Name)+"") %>"><%}else{ %><a href="<%=Hdkm.URL%>" target="_blank"><%} %><span><%=Lang(Hdkm.Name)%></span></a>
<%Hdkm_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int AuPE_index=1;
List<Lebi_Page> AuPEs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page AuPE in AuPEs){%>
<a class="menu" href="<%=URL("","",AuPE.url)%>" target="<%=AuPE.target%>"><span style="color:<%=AuPE.NameColor%>"><%=AuPE.Name%></span></a> 
<%AuPE_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int AHAS_index=1;
List<Lebi_FriendLink> AHASs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink AHAS in AHASs){%>
            
                <% if (AHAS.Logo != "" && AHAS.IsPic == 1){ %><a href="<%=AHAS.Url%>" target="_blank"><img src="<%=Image(AHAS.Logo) %>" alt="<%=AHAS.Name%>" /></a><%}else{%><a href="<%=AHAS.Url%>" target="_blank"><%=AHAS.Name%></a><%} %>     
