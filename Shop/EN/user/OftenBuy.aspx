...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> Norrs=Languages();RecordCount=Norrs.Count;int Norr_index=1;
foreach (DB.LebiShop.Lebi_Language Norr in Norrs){%>
            <dd <%if (Norr_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=Norr.id%>,'<%=Norr.Code%>','<%=Norr.Path%>');"><%if (Norr.ImageUrl!=""){%><img src="<%=Image(Norr.ImageUrl) %>" /><%}%><%=Norr.Name%></a></dd>
            <%Norr_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int decw_index=1;
List<Lebi_Currency> decws = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency decw in decws){%>
            <dd <%if (decw_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=decw.id%>,'<%=decw.Code%>',<%=decw.ExchangeRate%>,'<%=decw.Msige%>','<%=decw.DecimalLength%>');"><%=decw.Code%></a></dd>
            <%decw_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int IHDt_index=1;
List<Lebi_Searchkey> IHDts = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey IHDt in IHDts){%><%=Lang(IHDt.Name)%><%IHDt_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int Ycxn_index=1;
List<Lebi_Searchkey> Ycxns = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey Ycxn in Ycxns){%>
<%if (Ycxn.Type==1){ %><a href="<%=URL("P_Search",""+Lang(Ycxn.Name)+"") %>"><%}else{ %><a href="<%=Ycxn.URL%>" target="_blank"><%} %><span><%=Lang(Ycxn.Name)%></span></a>
<%Ycxn_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int fOpo_index=1;
List<Lebi_Page> fOpos = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page fOpo in fOpos){%>
<a class="menu" href="<%=URL("","",fOpo.url)%>" target="<%=fOpo.target%>"><span style="color:<%=fOpo.NameColor%>"><%=fOpo.Name%></span></a> 
<%fOpo_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int kbAR_index=1;
List<Lebi_FriendLink> kbARs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink kbAR in kbARs){%>
            
                <% if (kbAR.Logo != "" && kbAR.IsPic == 1){ %><a href="<%=kbAR.Url%>" target="_blank"><img src="<%=Image(kbAR.Logo) %>" alt="<%=kbAR.Name%>" /></a><%}else{%><a href="<%=kbAR.Url%>" target="_blank"><%=kbAR.Name%></a><%} %>     
