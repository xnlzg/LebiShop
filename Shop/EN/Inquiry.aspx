...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> smlxs=Languages();RecordCount=smlxs.Count;int smlx_index=1;
foreach (DB.LebiShop.Lebi_Language smlx in smlxs){%>
            <dd <%if (smlx_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=smlx.id%>,'<%=smlx.Code%>','<%=smlx.Path%>');"><%if (smlx.ImageUrl!=""){%><img src="<%=Image(smlx.ImageUrl) %>" /><%}%><%=smlx.Name%></a></dd>
            <%smlx_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int iurY_index=1;
List<Lebi_Currency> iurYs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency iurY in iurYs){%>
            <dd <%if (iurY_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=iurY.id%>,'<%=iurY.Code%>',<%=iurY.ExchangeRate%>,'<%=iurY.Msige%>','<%=iurY.DecimalLength%>');"><%=iurY.Code%></a></dd>
            <%iurY_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int tvSy_index=1;
List<Lebi_Searchkey> tvSys = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey tvSy in tvSys){%><%=Lang(tvSy.Name)%><%tvSy_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int wdSb_index=1;
List<Lebi_Searchkey> wdSbs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey wdSb in wdSbs){%>
<%if (wdSb.Type==1){ %><a href="<%=URL("P_Search",""+Lang(wdSb.Name)+"") %>"><%}else{ %><a href="<%=wdSb.URL%>" target="_blank"><%} %><span><%=Lang(wdSb.Name)%></span></a>
<%wdSb_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int wsKB_index=1;
List<Lebi_Page> wsKBs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page wsKB in wsKBs){%>
<a class="menu" href="<%=URL("","",wsKB.url)%>" target="<%=wsKB.target%>"><span style="color:<%=wsKB.NameColor%>"><%=wsKB.Name%></span></a> 
<%wsKB_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int KkLV_index=1;
List<Lebi_FriendLink> KkLVs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink KkLV in KkLVs){%>
            
                <% if (KkLV.Logo != "" && KkLV.IsPic == 1){ %><a href="<%=KkLV.Url%>" target="_blank"><img src="<%=Image(KkLV.Logo) %>" alt="<%=KkLV.Name%>" /></a><%}else{%><a href="<%=KkLV.Url%>" target="_blank"><%=KkLV.Name%></a><%} %>     
