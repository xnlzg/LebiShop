...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> sUCxs=Languages();RecordCount=sUCxs.Count;int sUCx_index=1;
foreach (DB.LebiShop.Lebi_Language sUCx in sUCxs){%>
            <dd <%if (sUCx_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=sUCx.id%>,'<%=sUCx.Code%>','<%=sUCx.Path%>');"><%if (sUCx.ImageUrl!=""){%><img src="<%=Image(sUCx.ImageUrl) %>" /><%}%><%=sUCx.Name%></a></dd>
            <%sUCx_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int FANU_index=1;
List<Lebi_Currency> FANUs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency FANU in FANUs){%>
            <dd <%if (FANU_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=FANU.id%>,'<%=FANU.Code%>',<%=FANU.ExchangeRate%>,'<%=FANU.Msige%>','<%=FANU.DecimalLength%>');"><%=FANU.Code%></a></dd>
            <%FANU_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int LteC_index=1;
List<Lebi_Searchkey> LteCs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey LteC in LteCs){%><%=Lang(LteC.Name)%><%LteC_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int sfbo_index=1;
List<Lebi_Searchkey> sfbos = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey sfbo in sfbos){%>
<%if (sfbo.Type==1){ %><a href="<%=URL("P_Search",""+Lang(sfbo.Name)+"") %>"><%}else{ %><a href="<%=sfbo.URL%>" target="_blank"><%} %><span><%=Lang(sfbo.Name)%></span></a>
<%sfbo_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int bbRs_index=1;
List<Lebi_Page> bbRss = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page bbRs in bbRss){%>
<a class="menu" href="<%=URL("","",bbRs.url)%>" target="<%=bbRs.target%>"><span style="color:<%=bbRs.NameColor%>"><%=bbRs.Name%></span></a> 
<%bbRs_index++;}%>
...
<div class="mc">
    ...
    if (type !=""){
        ...
        %>
    <%Table="Lebi_Page";Where=""+strwhere+"";PageSize=999;pageindex=Rint("page");RecordCount=B_Lebi_Page.Counts(Where);int KaSr_index=1;
List<Lebi_Page> KaSrs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page KaSr in KaSrs){%>
    <div class="nodeitem clearfix" id="help<%=KaSr.id%>">
    <div class="mt">
        <h2><font style="color:<%=KaSr.NameColor%>"><%=KaSr.Name%></font></h2>
    </div>
    <%if (KaSr.Description != ""){ %><div class="tips"><%=KaSr.Description%></div><%} %>
    <div class="content">
    <%=ImgSrc(KaSr.Content,CurrentLanguage.IsLazyLoad) %>
    </div>
    </div>
    <%KaSr_index++;}%>
    <%} %>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int ywtB_index=1;
List<Lebi_FriendLink> ywtBs = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink ywtB in ywtBs){%>
            
                <% if (ywtB.Logo != "" && ywtB.IsPic == 1){ %><a href="<%=ywtB.Url%>" target="_blank"><img src="<%=Image(ywtB.Logo) %>" alt="<%=ywtB.Name%>" /></a><%}else{%><a href="<%=ywtB.Url%>" target="_blank"><%=ywtB.Name%></a><%} %>     
