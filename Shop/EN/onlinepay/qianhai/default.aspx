...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> kOtBs=Languages();RecordCount=kOtBs.Count;int kOtB_index=1;
foreach (DB.LebiShop.Lebi_Language kOtB in kOtBs){%>
            <dd <%if (kOtB_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=kOtB.id%>,'<%=kOtB.Code%>','<%=kOtB.Path%>');"><%if (kOtB.ImageUrl!=""){%><img src="<%=Image(kOtB.ImageUrl) %>" /><%}%><%=kOtB.Name%></a></dd>
            <%kOtB_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int jqYv_index=1;
List<Lebi_Currency> jqYvs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency jqYv in jqYvs){%>
            <dd <%if (jqYv_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=jqYv.id%>,'<%=jqYv.Code%>',<%=jqYv.ExchangeRate%>,'<%=jqYv.Msige%>','<%=jqYv.DecimalLength%>');"><%=jqYv.Code%></a></dd>
            <%jqYv_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int CBFc_index=1;
List<Lebi_Searchkey> CBFcs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey CBFc in CBFcs){%><%=Lang(CBFc.Name)%><%CBFc_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int HzeD_index=1;
List<Lebi_Searchkey> HzeDs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey HzeD in HzeDs){%>
<%if (HzeD.Type==1){ %><a href="<%=URL("P_Search",""+Lang(HzeD.Name)+"") %>"><%}else{ %><a href="<%=HzeD.URL%>" target="_blank"><%} %><span><%=Lang(HzeD.Name)%></span></a>
<%HzeD_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int gApi_index=1;
List<Lebi_Page> gApis = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page gApi in gApis){%>
<a class="menu" href="<%=URL("","",gApi.url)%>" target="<%=gApi.target%>"><span style="color:<%=gApi.NameColor%>"><%=gApi.Name%></span></a> 
<%gApi_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int yXmt_index=1;
List<Lebi_FriendLink> yXmts = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink yXmt in yXmts){%>
            
                <% if (yXmt.Logo != "" && yXmt.IsPic == 1){ %><a href="<%=yXmt.Url%>" target="_blank"><img src="<%=Image(yXmt.Logo) %>" alt="<%=yXmt.Name%>" /></a><%}else{%><a href="<%=yXmt.Url%>" target="_blank"><%=yXmt.Name%></a><%} %>     
