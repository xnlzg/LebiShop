...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> wAKls=Languages();RecordCount=wAKls.Count;int wAKl_index=1;
foreach (DB.LebiShop.Lebi_Language wAKl in wAKls){%>
            <dd <%if (wAKl_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=wAKl.id%>,'<%=wAKl.Code%>','<%=wAKl.Path%>');"><%if (wAKl.ImageUrl!=""){%><img src="<%=Image(wAKl.ImageUrl) %>" /><%}%><%=wAKl.Name%></a></dd>
            <%wAKl_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int vOLo_index=1;
List<Lebi_Currency> vOLos = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency vOLo in vOLos){%>
            <dd <%if (vOLo_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=vOLo.id%>,'<%=vOLo.Code%>',<%=vOLo.ExchangeRate%>,'<%=vOLo.Msige%>','<%=vOLo.DecimalLength%>');"><%=vOLo.Code%></a></dd>
            <%vOLo_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int cIIO_index=1;
List<Lebi_Searchkey> cIIOs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey cIIO in cIIOs){%><%=Lang(cIIO.Name)%><%cIIO_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int SAVz_index=1;
List<Lebi_Searchkey> SAVzs = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey SAVz in SAVzs){%>
<%if (SAVz.Type==1){ %><a href="<%=URL("P_Search",""+Lang(SAVz.Name)+"") %>"><%}else{ %><a href="<%=SAVz.URL%>" target="_blank"><%} %><span><%=Lang(SAVz.Name)%></span></a>
<%SAVz_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int HUFA_index=1;
List<Lebi_Page> HUFAs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page HUFA in HUFAs){%>
<a class="menu" href="<%=URL("","",HUFA.url)%>" target="<%=HUFA.target%>"><span style="color:<%=HUFA.NameColor%>"><%=HUFA.Name%></span></a> 
<%HUFA_index++;}%>
...
<div class="mc clearfix">
    <ul>
        <%Table="Lebi_Page";Where="Node_id in (select id from Lebi_Node where parentid = "+ Node("Info").id +") and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Count_Views desc,id desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int YvCz_index=1;
List<Lebi_Page> YvCzs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page YvCz in YvCzs){%>
        <li class="text"><a href="<%=URL("P_ArticleDetails",YvCz.id,YvCz.url)%>" target="<%=YvCz.target%>" title="<%=YvCz.Name%>"><span style="color:<%=YvCz.NameColor%>"><%=YvCz.Name%></span></a></li>
        <%YvCz_index++;}%>
    </ul>
</div>
</div>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int Mddw_index=1;
List<Lebi_FriendLink> Mddws = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink Mddw in Mddws){%>
            
                <% if (Mddw.Logo != "" && Mddw.IsPic == 1){ %><a href="<%=Mddw.Url%>" target="_blank"><img src="<%=Image(Mddw.Logo) %>" alt="<%=Mddw.Name%>" /></a><%}else{%><a href="<%=Mddw.Url%>" target="_blank"><%=Mddw.Name%></a><%} %>     
