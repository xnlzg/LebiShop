...
<div class="language">
    <ul class="droplanguage">
        <li class="language_li"><a class="noclick"><span>Site Language：</span><s><%if (CurrentLanguage.ImageUrl!=""){%><img src="<%=Image(CurrentLanguage.ImageUrl) %>" /><%}%><%=CurrentLanguage.Name %></s></a><dl
            ...
            <%List<DB.LebiShop.Lebi_Language> LiEus=Languages();RecordCount=LiEus.Count;int LiEu_index=1;
foreach (DB.LebiShop.Lebi_Language LiEu in LiEus){%>
            <dd <%if (LiEu_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetLanguage(<%=LiEu.id%>,'<%=LiEu.Code%>','<%=LiEu.Path%>');"><%if (LiEu.ImageUrl!=""){%><img src="<%=Image(LiEu.ImageUrl) %>" /><%}%><%=LiEu.Name%></a></dd>
            <%LiEu_index++;}%>
        </dl>
        </li>
    </ul>
...
<div class="currency">
    <ul class="dropcurrency">
        <li class="currency_li"><a class="noclick"><span>Currency：</span><s><%=CurrentCurrency.Code %></s></a><dl
            ...
            <%Table="Lebi_Currency";Where="";Order="Sort desc";PageSize=20;pageindex=Rint("page");RecordCount=B_Lebi_Currency.Counts(Where);int XslU_index=1;
List<Lebi_Currency> XslUs = B_Lebi_Currency.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Currency XslU in XslUs){%>
            <dd <%if (XslU_index==RecordCount){%>class="last" <%} %>>
                <a href="javascript:SetCurrency(<%=XslU.id%>,'<%=XslU.Code%>',<%=XslU.ExchangeRate%>,'<%=XslU.Msige%>','<%=XslU.DecimalLength%>');"><%=XslU.Code%></a></dd>
            <%XslU_index++;}%>
        </dl>
        </li>
    </ul>
...
<script type="text/javascript">
    $(function () {
        blurInput('#keyword', '<%if (Rstring("Keyword")!=""){ %><%=Rstring("Keyword")%><%}else{%><%Table="Lebi_Searchkey";Where="Type=1";Order="Sort desc,id desc";PageSize=1;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int arSi_index=1;
List<Lebi_Searchkey> arSis = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey arSi in arSis){%><%=Lang(arSi.Name)%><%arSi_index++;}%><%} %>');
...
<%Table="Lebi_Searchkey";Where="";Order="Sort desc,id desc";PageSize=5;pageindex=1;RecordCount=B_Lebi_Searchkey.Counts(Where);int fqPy_index=1;
List<Lebi_Searchkey> fqPys = B_Lebi_Searchkey.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Searchkey fqPy in fqPys){%>
<%if (fqPy.Type==1){ %><a href="<%=URL("P_Search",""+Lang(fqPy.Name)+"") %>"><%}else{ %><a href="<%=fqPy.URL%>" target="_blank"><%} %><span><%=Lang(fqPy.Name)%></span></a>
<%fqPy_index++;}%>
</div>
...
<%Table="Lebi_Page";Where="Node_id="+Node("HeadMenu").id+" and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc";PageSize=10;pageindex=1;RecordCount=B_Lebi_Page.Counts(Where);int bpez_index=1;
List<Lebi_Page> bpezs = B_Lebi_Page.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_Page bpez in bpezs){%>
<a class="menu" href="<%=URL("","",bpez.url)%>" target="<%=bpez.target%>"><span style="color:<%=bpez.NameColor%>"><%=bpez.Name%></span></a> 
<%bpez_index++;}%>
...
<div class="footer">
    ...
    <div class="copyright f11 footer_logos">
        <div class="footer_logos-list">
            <%Table="Lebi_FriendLink";Where="IsShow=1 and ','+Language_ids+',' like '%," + CurrentLanguage.id + ",%'";Order="Sort desc,id desc";PageSize=7;pageindex=1;RecordCount=B_Lebi_FriendLink.Counts(Where);int yVsp_index=1;
List<Lebi_FriendLink> yVsps = B_Lebi_FriendLink.GetList(Where, Order,PageSize ,pageindex);foreach (Lebi_FriendLink yVsp in yVsps){%>
            
                <% if (yVsp.Logo != "" && yVsp.IsPic == 1){ %><a href="<%=yVsp.Url%>" target="_blank"><img src="<%=Image(yVsp.Logo) %>" alt="<%=yVsp.Name%>" /></a><%}else{%><a href="<%=yVsp.Url%>" target="_blank"><%=yVsp.Name%></a><%} %>     
